...
class AnalysisEngine:
    ...
    def calculate_financial_indicators(self,
        ...
        try:
            ...
            print(f"MOTEUR (Mensuel): Données énergétiques agrégées prêtes ({len(data_agg)} lignes).")
            ...
            for month_idx in range(num_months_simulation):
                ...
                # Resultat Net = EBT (l'impôt est flux séparé)
                resultat_net_mois = ebt_mois
                monthly_results_df.loc[current_month_end_date, 'Resultat_Net'] = resultat_net_mois

                # --- Calcul Flux Mensuels (MODIFIER FCFE) ---
                # FCFE = EBT - Paiement Impôt + Amortissement - Remboursement Principal
                fcfe_mois = (ebt_mois
                             - tax_payment_this_month # Soustraire le paiement effectif
                ...
                # OCF Projet (inchangé, basé sur taux standard)
                ...
                # Service Dette et Solde Dette (inchangé)
