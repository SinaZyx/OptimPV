...
class ConfigModule:
    ...
    def initialize_default_config(self):
        ...
        default_power = 500.0 # Exemple de puissance par défaut
        ...
        st.session_state.config = {
            ...
            "tarif_oa_indexe_inflation": False, # Le flag pour activer/désactiver
            "taux_inflation_tarif_oa": 1.5, # Taux spécifique pour l'OA (en %)
            # --- FIN AJOUT ---
            ...
            "source_prix_autoconso": "tarif_edf", # MODIFIÉ: Source pour valoriser l'autoconsommation ('prix_initial', 'tarif_edf', 'tarif_oa')
            # --- FIN AJOUT ---
    ...
    def show_ui(self):
        ...
        with tab1:
            ...
            with col1:
                ...
                index_oa_checkbox = st.checkbox( 
                    ...
                    value=config.get("tarif_oa_indexe_inflation", False), 
                ...
                if index_oa_checkbox: # Afficher seulement si la case est cochée
                    ...
                    st.session_state.config["taux_inflation_tarif_oa"] = st.number_input(
                        ...
                        value=float(config.get("taux_inflation_tarif_oa", 1.5)), 
                        step=0.1,
        ...
        with tab2:
            ...
            with col1:
                ...
                puissance_kwc = st.number_input(
                    ...
                    value=float(config.get("puissance_kwc", 500.0)), step=1.0, 
            ...
            if source_prix_selection == 'prix_initial':
                ...
                oa_index_txt = " (indexé)" if config.get('tarif_oa_indexe_inflation', False) else " (non indexé)"
            ...
            with st.expander("🔧 Configuration Barème Prime à l'Investissement (€/kWc)"):
                ...
                with col_sub1:
                    st.session_state.config["subvention_rate_le9"] = st.number_input(
                        ...
                        value=float(config.get("subvention_rate_le9", 300.0)), step=1.0, 
                    ...
                    st.session_state.config["subvention_rate_le36"] = st.number_input(
                        ...
                        value=float(config.get("subvention_rate_le36", 230.0)), step=1.0, 
                ...
                with col_sub2:
                    st.session_state.config["subvention_rate_le100"] = st.number_input(
                        ...
                        value=float(config.get("subvention_rate_le100", 190.0)), step=1.0, 
                    ...
                    st.session_state.config["subvention_rate_le500"] = st.number_input(
                        ...
                        value=float(config.get("subvention_rate_le500", 95.0)), step=1.0, 
            ...
            with col2_opex:
                ...
                puissance_val = config.get("puissance_kwc", 500.0)
                ...
                if opex_onduleur_provision:
                     opex_onduleur_lifetime = st.number_input(
                        ...
                        value=config.get("opex_onduleur_lifetime", 14), step=1, 
        ...
        with st.expander("Afficher le récapitulatif des paramètres"):
            ...
            st.write(f"Prix min de revente: {st.session_state.config.get('prix_min_revente', 0.15)} €/kWh")
            st.write(f"Prix max de revente: {st.session_state.config.get('prix_max_revente', 0.21)} €/kWh")
            st.write(f"Pas d'optimisation: {st.session_state.config.get('pas_optimisation', 0.01)} €/kWh")
    ...
    def ensure_config_defaults(self):
        ...
        default_power = 500.0 # Même valeur que dans init
        default_config_dict = { 
             "nom_projet": "Projet Photovoltaïque", "localisation": "Marseille, France", 
             "puissance_kwc": default_power, 
             "tarif_oa_bracket_le9": 0.0761, # Exemple de défaut (à synchroniser)
             "tarif_oa_bracket_le100": 0.0761, # Exemple de défaut (à synchroniser)
             "tarif_oa_bracket_gt100": 0.0600, # Exemple de défaut (à synchroniser)
             # Initialise tarif_oa basé sur puissance ET paliers par défaut
             # Note: ici on ne peut pas appeler default_tarif_oa facilement sans risque
             # On met une valeur par défaut simple, elle sera écrasée si les paliers existent.
             "tarif_oa": 0.0600, # Valeur basée sur default_power > 100kWc (à synchroniser)
             "tarif_oa_indexe_inflation": False, 
             "taux_inflation_tarif_oa": 1.5,
             "subvention_rate_le9": 300.0,
             "subvention_rate_le36": 230.0,
             "subvention_rate_le100": 190.0,
             "subvention_rate_le500": 95.0,
             ...
             "opex_onduleur_lifetime": 14, "date_debut_ppa": "2023-01-01", 
             "duree_ppa": 240, "duree_construction": 12, 
             
             # --- AJOUT DES CLES MANQUANTES ICI AUSSI ---
             "amortissement_duree": 15,
             "valeur_residuelle_pct": 0.0,
             "cout_demantelement_pct": 0.05, # CORRIGÉ
             "source_prix_autoconso": "tarif_edf", # MODIFIÉ
             # --- FIN AJOUT ---
