...
if not logger.handlers:
    ...
    if not logging.getLogger().handlers: # Si le root logger n'a AUCUN handler
        ...
        log_file_path = "optim_pv_enginev5.txt"
...
class AnalysisEngine:
    ...
    def calculate_financial_indicators(self,
        ...
        try:
            ...
            # Supprimer l'appel initial à calculate_monthly_loan_schedule pour l'échéancier complet ici.
            # monthly_interest_paid, monthly_principal_paid, monthly_debt_balance = calculate_monthly_loan_schedule(...) # <--- SUPPRIMÉ

            # Initialiser les séries d'intérêts et principal payés à 0
            monthly_interest_paid_series = np.zeros(num_total_simulation_months)
            monthly_principal_paid_series = np.zeros(num_total_simulation_months)
            
            current_debt_drawn_balance = 0.0 # Solde de la dette réellement tirée
            accumulated_capitalized_interest = 0.0 # Pour les intérêts capitalisés pendant la construction
            ...
            capitalize_construction_interest = global_config.get("capitalize_construction_interest", True)
            ...
            for month_idx in range(num_total_simulation_months):
                ...
                if is_construction_phase:
                    ...
                    monthly_results_df.loc[current_month_end_date, 'CAPEX_Initial_Mensuel'] = 0.0

                # Calcul du tirage de dette pour le mois courant
                ...
                if is_construction_phase:
                    ...
                    elif debt_amount > 0 and duree_construction_cfg > 0 :
                        current_drawdown_for_month = debt_amount / duree_construction_cfg
                elif month_idx == duree_construction_cfg and capex_net_subvention <= 1e-6 and debt_amount > 0 and duree_construction_cfg == 0 : # Cas où tout est au premier mois et pas de CAPEX
                ...
                if is_construction_phase:
                    if current_debt_drawn_balance > 0:
                        interest_m = current_debt_drawn_balance * (taux_interet_dette_annual / 12.0)
                    
                    if capitalize_construction_interest:
                        accumulated_capitalized_interest += interest_m
                        monthly_interest_paid_series[month_idx] = 0.0
                    else:
                        monthly_interest_paid_series[month_idx] = interest_m
                    
                    monthly_principal_paid_series[month_idx] = 0.0
                
                else: # Phase d'exploitation
                    if not loan_amortization_schedule_generated and loan_active: # Vérifier loan_active
                        ...
                        if final_loan_principal_for_amortization > 1e-6 and loan_term_for_amortization_years > 0:
                            # Assurer que calculate_monthly_loan_schedule est disponible
                            op_interests, op_principals, _ = calculate_monthly_loan_schedule(
                            ...
                            operation_phase_loan_interests[:len(op_interests)] = op_interests
                            operation_phase_loan_principals[:len(op_principals)] = op_principals
                        
                        loan_amortization_schedule_generated = True

                    if loan_active: # Vérifier loan_active
                        operational_month_idx = month_idx - duree_construction_cfg
                        if operational_month_idx < len(operation_phase_loan_interests):
                            interest_m = operation_phase_loan_interests[operational_month_idx]
                            principal_m = operation_phase_loan_principals[operational_month_idx]
                    
                    monthly_interest_paid_series[month_idx] = interest_m
                    monthly_principal_paid_series[month_idx] = principal_m
                    current_debt_drawn_balance -= principal_m
                
                monthly_results_df.loc[current_month_end_date, 'Interets_Payes'] = monthly_interest_paid_series[month_idx]
                monthly_results_df.loc[current_month_end_date, 'Principal_Rembourse'] = monthly_principal_paid_series[month_idx]
                monthly_results_df.loc[current_month_end_date, 'Service_Dette'] = monthly_interest_paid_series[month_idx] + monthly_principal_paid_series[month_idx]
                ...
                if is_construction_phase:
                    ...
                    # Initialisation de df_hourly_month pour éviter les erreurs si le try échoue ou si ref_monthly_energy_slice est vide
                    df_hourly_month = pd.DataFrame(columns=['production', 'consumption']) 

                    try:
                        current_month_number = current_month_end_date.month
                        
                        if not reference_data_ts.empty and isinstance(reference_data_ts.index, pd.DatetimeIndex):
                            ref_monthly_energy_slice = reference_data_ts[reference_data_ts.index.month == current_month_number]
                        else:
                            ref_monthly_energy_slice = pd.DataFrame(columns=['production_kwh', 'consumption_kwh'])
                            logger.warning(f"CORE WARNING - Mois {current_month_number}: reference_data_ts est vide ou son index n'est pas un DatetimeIndex. Production/conso pour ce mois sera nulle.")

                        if not ref_monthly_energy_slice.empty:
                            prod_ref_s = ref_monthly_energy_slice['production_kwh']
                            cons_ref_s = ref_monthly_energy_slice['consumption_kwh']

                            prod_adj_s = prod_ref_s * degradation_factor_op_year_current * production_modifier_scenario
                            
                            df_hourly_month = pd.DataFrame({
                                'production': prod_adj_s,
                                'consumption': cons_ref_s 
                            }).fillna(0)

                            production_values = df_hourly_month['production'].values
                            consumption_values = df_hourly_month['consumption'].values
                        
                            autoconsommation_horaire_np = np.minimum(production_values, consumption_values)
                            surplus_horaire_np = production_values - autoconsommation_horaire_np
                            surplus_horaire_np = np.maximum(0, surplus_horaire_np)

                            prod_m_calc = production_values.sum()
                            cons_m_calc = consumption_values.sum()
                            auto_m_calc = autoconsommation_horaire_np.sum()
                            surplus_m_calc = surplus_horaire_np.sum()
                            
                            # Log de débogage principal conservé ici si besoin (peut être commenté plus tard)
                            # logger.info(f"CORE DEBUG DANS TRY - Mois {current_month_number} - prod_m_calc: {prod_m_calc:.2f}, cons_m_calc: {cons_m_calc:.2f}, auto_m_calc: {auto_m_calc:.2f}, surplus_m_calc: {surplus_m_calc:.2f}")
                        else:
                            logger.warning(f"CORE WARNING - Mois {current_month_number}: ref_monthly_energy_slice était vide après sélection. Production/conso pour ce mois sera nulle.")
                            prod_m_calc, cons_m_calc, auto_m_calc, surplus_m_calc = 0.0, 0.0, 0.0, 0.0

                    except Exception as e_autoc:
                        logger.error(f"ERREUR DANS CALCUL AUTOCONSOMMATION MENSUELLE - Mois {current_month_end_date.month if 'current_month_end_date' in locals() else 'inconnu'}: {e_autoc}")
                        logger.error(traceback.format_exc())
                        prod_m_calc, cons_m_calc, auto_m_calc, surplus_m_calc = 0.0, 0.0, 0.0, 0.0 # Assurer des valeurs nulles en cas d'erreur
                    
                    prod_m = prod_m_calc
                    cons_m = cons_m_calc
                    auto_m = auto_m_calc
                    surplus_m = surplus_m_calc
                    
                    # Assignations manquantes cruciales ajoutées ici:
                    ...
                    # Les logs CORE DEBUG AVANT LOG MENSUEL CALC et ULTIME VERIF peuvent être commentés/supprimés si cette section est stable
                    # logger.info(f"CORE DEBUG AVANT LOG MENSUEL CALC - Prod_m affectée: {prod_m:.2f}")
                    # valeur_prod_m_avant_log_final = prod_m 
                    # logger.info(f"ULTIME VERIF - prod_m juste avant log final: {valeur_prod_m_avant_log_final:.2f}")
                ...
                interest_m = monthly_interest_paid_series[month_idx] if month_idx < len(monthly_interest_paid_series) else 0.0
                principal_m = monthly_principal_paid_series[month_idx] if month_idx < len(monthly_principal_paid_series) else 0.0
                ...
                opex_m = monthly_results_df.loc[current_month_end_date, 'OPEX']
                turpe_m = monthly_results_df.loc[current_month_end_date, 'TURPE']

                vat_collectee_m = rev_total_m * vat_rate_operations
                vat_deductible_opex_turpe_m = (opex_m + turpe_m) * vat_rate_operations
                
                vat_deductible_capex_m = 0.0
                # La récupération de la TVA sur CAPEX se fait sur le total, au Xème mois (selon config)
                # tva_sur_capex_initial est calculé avant la boucle
                if month_idx == vat_capex_recovery_month_offset:
                    vat_deductible_capex_m = tva_sur_capex_initial
                
                # Calcul de la TVA nette pour le mois courant avant de considérer le report précédent
                tva_nette_du_mois_avant_report = vat_collectee_m - vat_deductible_opex_turpe_m - vat_deductible_capex_m
                
                # Solde de TVA en incluant le crédit reporté du mois précédent
                # vat_credit_carryforward est initialisé à 0.0 avant la boucle
                solde_tva_a_regulariser = tva_nette_du_mois_avant_report + vat_credit_carryforward

                vat_payment_this_month_final = 0.0
                nouveau_vat_credit_carryforward_pour_mois_suivant = 0.0

                # Lire les paramètres de remboursement depuis la configuration globale
                remboursement_credit_tva_actif = bool(global_config.get("remboursement_credit_tva_actif", True))
                seuil_declenchement_remboursement_tva = float(global_config.get("seuil_remboursement_tva", 500.0)) # Ex: seuil de crédit de 500€
                # Un délai de remboursement pourrait être ajouté ici (plus complexe)

                if solde_tva_a_regulariser > 0:
                    # TVA à payer
                    vat_payment_this_month_final = solde_tva_a_regulariser
                    nouveau_vat_credit_carryforward_pour_mois_suivant = 0.0
                else: # solde_tva_a_regulariser <= 0 (crédit de TVA pour le mois, ou augmentation du crédit existant)
                    if remboursement_credit_tva_actif and abs(solde_tva_a_regulariser) >= seuil_declenchement_remboursement_tva:
                        # Remboursement du crédit de TVA
                        vat_payment_this_month_final = solde_tva_a_regulariser # Sera négatif, donc un cash-in
                        nouveau_vat_credit_carryforward_pour_mois_suivant = 0.0 # Crédit apuré par remboursement
                    else:
                        # Pas de paiement (car crédit) et pas de remboursement (seuil non atteint ou option inactive)
                        vat_payment_this_month_final = 0.0
                        nouveau_vat_credit_carryforward_pour_mois_suivant = solde_tva_a_regulariser # Report du crédit
                
                # Mise à jour du crédit reporté pour le mois suivant
                vat_credit_carryforward = nouveau_vat_credit_carryforward_pour_mois_suivant

                # Stockage des résultats mensuels de TVA
                ...
                monthly_results_df.loc[current_month_end_date, 'VAT_Due_Mois'] = solde_tva_a_regulariser # Ce qui était dû/crédité avant paiement/remboursement
                monthly_results_df.loc[current_month_end_date, 'VAT_Payment'] = vat_payment_this_month_final # Le flux de cash net de TVA
                ...
                monthly_results_df.loc[current_month_end_date, 'Solde_Dette_Fin_Mois'] = monthly_results_df.loc[current_month_end_date, 'Solde_Dette_Fin_Mois'] - monthly_principal_paid_series[month_idx] if month_idx < len(monthly_principal_paid_series) else 0.0
            ...
            # Récupération des séries mensuelles nécessaires
            ebitda_mensuel_series = monthly_results_df['EBITDA'].fillna(0) 
            amortissement_mensuel_series = monthly_results_df['Amortissement'].fillna(0)
            capex_net_mensuel_series = monthly_results_df['CAPEX_Initial_Mensuel'].fillna(0)
            delta_bfr_mensuel_series = monthly_results_df['Delta_BFR_Mensuel'].fillna(0)
            vat_payment_mensuel_series = monthly_results_df['VAT_Payment'].fillna(0)
            ebit_mensuel_series = monthly_results_df['EBIT'].fillna(0)

            # --- NOUVEAU BLOC POUR CALCULER L'IMPÔT SUR EBIT ---
            impot_sur_ebit_mensuel_series = pd.Series(0.0, index=monthly_results_df.index)
            ebit_annuel_series = ebit_mensuel_series.groupby(ebit_mensuel_series.index.year).sum()
            loss_carryforward_balance_for_ebit_calc = 0.0

            for year in ebit_annuel_series.index:
                current_year_ebit = ebit_annuel_series.loc[year]
                taxable_ebit_for_year, loss_carryforward_balance_for_ebit_calc = tax_engine.apply_loss_carryforward_cap(
                    current_year_ebit, 
                    loss_carryforward_balance_for_ebit_calc 
                )
                impot_theorique_annuel_sur_ebit = tax_engine.calculate_corporate_tax_pme(taxable_ebit_for_year)
                
                if impot_theorique_annuel_sur_ebit > 0:
                    ebit_mensuels_annee_en_cours = ebit_mensuel_series[ebit_mensuel_series.index.year == year]
                    total_ebit_positif_annee = ebit_mensuels_annee_en_cours[ebit_mensuels_annee_en_cours > 0].sum()

                    for month_date in ebit_mensuels_annee_en_cours.index:
                        if total_ebit_positif_annee > 1e-6:
                            prorata = ebit_mensuels_annee_en_cours.loc[month_date] / total_ebit_positif_annee if ebit_mensuels_annee_en_cours.loc[month_date] > 0 else 0
                            impot_sur_ebit_mensuel_series.loc[month_date] = impot_theorique_annuel_sur_ebit * prorata
                        elif len(ebit_mensuels_annee_en_cours[ebit_mensuels_annee_en_cours > 0]) > 0 :
                             impot_sur_ebit_mensuel_series.loc[month_date] = (impot_theorique_annuel_sur_ebit / len(ebit_mensuels_annee_en_cours[ebit_mensuels_annee_en_cours > 0])) \
                                                                            if ebit_mensuels_annee_en_cours.loc[month_date] > 0 else 0
                        else: 
                             impot_sur_ebit_mensuel_series.loc[month_date] = impot_theorique_annuel_sur_ebit / 12.0
            
            nopat_mensuel = ebit_mensuel_series - impot_sur_ebit_mensuel_series
            # --- FIN DU NOUVEAU BLOC ---

            # Nouvelle Formule de OCF_Projet (FCFF)
            #    On inclut maintenant delta_bfr_mensuel_series et vat_payment_mensuel_series
            monthly_results_df['OCF_Projet'] = (
                nopat_mensuel
                + amortissement_mensuel_series    # Charge non décaissée
                - capex_net_mensuel_series        # Investissements (déjà nets de subvention)
                - delta_bfr_mensuel_series        # Variation du BFR (un "+" si le BFR diminue)
                - vat_payment_mensuel_series      # Paiement net de TVA (un "+" si crédit de TVA encaissé)
            )
            # --- FIN DU CALCUL AMÉLIORÉ DE OCF_Projet ---
            ...
            # NPV, IRR, ROI Equity
            npv_equity = npf.npv(re_monthly, equity_cash_flows_monthly) if pd.notna(re_monthly) else np.nan # Calcul normal de la VAN

            if is_equity_negligible:
                irr_equity = np.nan  # Si équité négligeable, TRI est N/A ou infini
                # roi_equity est déjà géré pour être np.nan si net_equity_investment est trop petit
            else:
            ...
            payback_equity_months = calculate_payback_months(equity_cash_flows_monthly) # Laissez ce calcul

            if is_equity_negligible:
                payback_equity_years = np.nan # Si équité négligeable, Payback est N/A
            else:
                # Si l'équité est significative mais le payback est instantané (0 mois),
                # cela signifie que les FCFE sont positifs dès le début.
