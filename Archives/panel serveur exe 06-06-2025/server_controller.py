...
class OptimPVController:
    ...
    def _run_streamlit(self):
        ...
        try:
            ...
            # SOLUTION: Utiliser directement subprocess au lieu de stcli.main()
            # pour éviter les problèmes de configuration interne de Streamlit
            logging.info("Lancement de Streamlit via subprocess...")
            
            # Préparer la commande Streamlit - CORRECTION: utiliser python.exe au lieu de l'exe
            if getattr(sys, 'frozen', False):
                # En mode PyInstaller, trouver python.exe dans l'environnement système
                python_exe = "python.exe"  # Utiliser python du PATH
                # Alternative: chercher dans les emplacements communs
                common_python_paths = [
            ...
            logging.info(f"Commande Streamlit: {' '.join(cmd)}")
            
            # Lancer Streamlit dans un sous-processus
            try:
                # Variables d'environnement pour forcer le mode production
                env = os.environ.copy()
                env['STREAMLIT_SERVER_DEVELOPMENT_MODE'] = 'false'
                ...
                # Attendre que le processus soit stable
                import time
                time.sleep(2)
                ...
                logging.error(f"Erreur lors du lancement de Streamlit: {e}")
                self.server_running = False
