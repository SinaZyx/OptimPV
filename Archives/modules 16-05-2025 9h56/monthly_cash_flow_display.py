...
try:
    from financial_display_utils import format_value, load_table_map # get_display_name_from_map n'est pas utilisé ici
except ImportError:
    # Fallback si l'import direct échoue (utile pour certains environnements de test)
    from .financial_display_utils import format_value, load_table_map
...
def display_cash_flow_statement_restructured(results: dict | None, selected_year: int):
    ...
    table_css_styles = """
    <style>
        table.dataframe-cfs { width:100%; border-collapse: collapse; font-size:0.85em; margin-bottom: 15px; }
        table.dataframe-cfs th { text-align: right; padding: 6px 8px; border-bottom: 1px solid #bbb; background-color: #eef; font-weight: bold; position: sticky; top: 0; z-index: 1; }
        table.dataframe-cfs td { text-align: right; padding: 5px 8px; border-bottom: 1px dotted #ddd; }
        table.dataframe-cfs td:first-child, table.dataframe-cfs th:first-child { text-align: left; font-weight: normal; }
        .group-header-cfs td { background-color: #d0d9ff !important; font-weight: bold; text-align: left !important; padding: 8px 4px !important; border-top: 2px solid #aaa; border-bottom: 1px solid #bbb; }
        .total-row-style td { font-weight: bold !important; border-top: 1px solid #aaa; }
        .subtotal-row-style td { font-style: italic; } /* Peut être combiné avec total-row-style */
        .main-total-row-style td { font-weight: bold !important; border-top: 2px solid #888; border-bottom: 2px solid #888; background-color: #e0e7ff; }
        .separator-row-cfs-subtle td { height: 5px; padding:0 !important; border:0 !important; background-color: #f0f0f0 !important; }
        .separator-row-cfs-strong td { height: 10px; padding:0 !important; border:0 !important; background-color: #e0e0e0 !important; }
    ...
    <strong>Notes :</strong>
    <ul>
        <li>FTE : Flux de Trésorerie d'Exploitation</li>
        <li>FTI : Flux de Trésorerie d'Investissement</li>
        <li>FTF : Flux de Trésorerie de Financement</li>
