    ...
    def simulate_selling_price(self, scenario_name, target_irr=None, target_npv=None):
        ...
        try:
            ...
            if target_npv is not None and abs(target_npv) < 1e-9:
                ...
                if abs(discounted_auto) < 1e-9: # Vérifier si le dénominateur est quasi nul
                    st.error("Autoconsommation actualisée quasi nulle, impossible de calculer le prix plancher P.")
                    prix_plancher = None # Ou une autre valeur indiquant une erreur
                else:
                    prix_plancher = (
                        capex_net
                      + discounted_opex
                ...
                # Vérifier si le calcul du prix plancher a réussi avant de continuer
                if prix_plancher is None:
                    return None
