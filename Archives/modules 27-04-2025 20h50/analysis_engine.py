...
class AnalysisEngine:
    ...
    def __init__(self, config: dict, scenarios: dict, sites_data: dict[str, pd.DataFrame]):
        ...
        # Vérifier le type de sites_data et s'assurer qu'il n'est pas vide
        ...
        # Vérifier que chaque valeur dans le dictionnaire est un DataFrame non vide
        for site_id, df in sites_data.items():
            ...
            # Vérifier la présence des colonnes essentielles dans chaque DataFrame
            required_cols = ['Temps', 'production_kwh', 'consumption_kwh']
            if not all(col in df.columns for col in required_cols):
                raise ValueError(f"Colonnes manquantes {set(required_cols) - set(df.columns)} pour le site '{site_id}'")

        self.config = config
        self.scenarios = scenarios
        # Stocker le dictionnaire des données de site
    ...
    def calculate_financial_indicators(self, scenario_name: str, prix_revente: float | None = None, override_source_prix_autoconso: str | None = None) -> dict | None:
        ...
        try:
            # --- DEBUT BLOC AGRÉGATION MODIFIÉ --- 
            if not self.sites_data:
                raise ValueError("Aucune donnée de site disponible pour l'analyse.")

            print(f"MOTEUR: Agrégation des données pour {len(self.sites_data)} site(s).")

            # Initialiser un DataFrame agrégé vide avec la colonne Temps du *premier* site comme référence
            # ATTENTION: Si les sites n'ont pas de période temporelle commune, cela peut échouer ou être incomplet.
            # Une approche plus robuste consisterait à trouver la plage temporelle globale min/max
            # et créer un DatetimeIndex complet, mais gardons cela simple pour l'instant.
            site_ids = list(self.sites_data.keys())
            if not site_ids:
                 raise ValueError("La liste des sites est vide après récupération des clés.")
                 
            first_site_df = self.sites_data[site_ids[0]]
            # S'assurer que le premier DF a une colonne Temps valide
            if 'Temps' not in first_site_df.columns or not pd.api.types.is_datetime64_any_dtype(first_site_df['Temps']):
                 raise ValueError(f"La colonne 'Temps' du premier site ({site_ids[0]}) est invalide ou manquante.")
                 
            # Initialiser avec index Temps et colonnes à zéro
            aggregated_data = pd.DataFrame(
                {'production_kwh': 0.0, 'consumption_kwh': 0.0},
                index=first_site_df['Temps'].drop_duplicates().sort_values()
            )
            # S'assurer que l'index est bien un DatetimeIndex
            if not isinstance(aggregated_data.index, pd.DatetimeIndex):
                 aggregated_data.index = pd.to_datetime(aggregated_data.index)

            # Boucler sur chaque site et fusionner/ajouter ses données
            for site_id, df_site in self.sites_data.items():
                if 'Temps' not in df_site.columns or not pd.api.types.is_datetime64_any_dtype(df_site['Temps']):
                     print(f"AVERTISSEMENT MOTEUR: Données temporelles invalides pour le site '{site_id}', ce site sera ignoré dans l'agrégation.")
                     continue # Ignorer ce site
                
                # Préparer le DataFrame du site pour la fusion/addition
                df_to_add = df_site[['Temps', 'production_kwh', 'consumption_kwh']]
                df_to_add = df_to_add.set_index('Temps')
                
                # S'assurer qu'il n'y a pas d'index dupliqué dans df_to_add avant d'ajouter
                # Ceci peut arriver si le fichier source a des doublons temporels
                if df_to_add.index.has_duplicates:
                     print(f"AVERTISSEMENT MOTEUR: Doublons temporels détectés dans le site '{site_id}'. Agrégation par somme sur les doublons.")
                     df_to_add = df_to_add.groupby(df_to_add.index).sum()

                # Additionner les colonnes en alignant sur l'index Temps
                # Utiliser reindex().add() pour gérer les cas où les index diffèrent largement
                aggregated_data = aggregated_data.add(df_to_add, fill_value=0)

            # Réinitialiser l'index pour retrouver la colonne Temps
            aggregated_data = aggregated_data.reset_index().rename(columns={'index': 'Temps'})

            # Assigner le résultat à la variable 'data' utilisée par la suite
            data = aggregated_data.sort_values(by='Temps').reset_index(drop=True) # Assurer l'ordre chronologique final

            print(f"MOTEUR: Données agrégées : {len(data)} lignes.")
            if data.empty:
                raise ValueError("Les données agrégées sont vides après la fusion.")
            
            # --- AJOUTER CET AVERTISSEMENT --- 
            if len(self.sites_data) > 1:
                print("*********************************************************************")
                print("AVERTISSEMENT MOTEUR: Analyse multi-sites en cours.")
                print("Les calculs financiers (CAPEX, OPEX, Dette, Subvention, Amortissement)")
                print("utilisent les paramètres GLOBAUX définis dans la configuration.")
                print("Ces résultats sont représentatifs SEULEMENT si la configuration globale")
                print("reflète les coûts TOTAUX de l'opération collective, ou si elle")
                print("modélise un producteur unique face à la consommation agrégée.")
                print("Pour une analyse multi-sites hétérogène précise, une configuration")
                print("par site serait nécessaire.")
                print("*********************************************************************")
            # --- FIN AJOUT ---
            
            # --- FIN BLOC AGRÉGATION MODIFIÉ --- 

            # Le reste de la fonction utilise maintenant le DataFrame agrégé 'data'
            # au lieu de self.processed_data

            # Utiliser self.config, self.scenarios
            if scenario_name not in self.scenarios:
                raise ValueError(f"Scénario '{scenario_name}' invalide.")
            # data = self.processed_data.copy() # <-- SUPPRIMER/COMMENTER CETTE LIGNE
            ...
            required_keys = [
                 ...
                 # --- AJOUT/MODIFICATION TURPE --- 
                 "turpe_prod_calculee", # Utiliser la clé de la valeur calculée
                 "turpe_indexe_inflation" # Garder la clé pour l'indexation
                 # --- FIN AJOUT/MODIFICATION TURPE --- 
            ]
            ...
            try:
                ...
                # --- AJOUT/MODIFICATION TURPE --- 
                turpe_initial = float(config.get("turpe_prod_calculee", -999.0)) # Lire la valeur calculée
                turpe_indexed = config.get("turpe_indexe_inflation", False)
                print(f"DEBUG ENGINE: turpe_initial lue depuis config = {turpe_initial}") # <-- AJOUTER
                # --- FIN AJOUT/MODIFICATION TURPE --- 
                
            except (ValueError, TypeError, KeyError) as e:
                raise ValueError(f"Erreur paramètre config: {e}") from e

            # --- Application modificateurs (utilise maintenant capex_base et opex_base NON mis à l'échelle) --- 
            ...
            # --- Calcul subventions (utilise maintenant puissance_kwc NON mis à l'échelle) --- 
            ...
            total_subvention = applicable_sub_rate * puissance_kwc # Utilise puissance_kwc de config
            # if num_sites > 0: print(f"  -> Subvention recalculée (base puissance {puissance_kwc:.2f} kWc): {total_subvention:.2f} €") # Log désactivé

            # --- Calculs financiers base (Equity - utilisent maintenant capex NON mis à l'échelle et subvention recalculée) --- 
            ...
            # La logique ici utilise maintenant 'data' (agrégé) au lieu de self.processed_data
            required_data_cols = ['Temps', 'production_kwh', 'consumption_kwh']
            if not all(col in data.columns for col in required_data_cols):
                # Utiliser data.columns ici pour le message d'erreur
                raise ValueError(f"Colonnes manquantes données agrégées: {set(required_data_cols) - set(data.columns)}")
            ...
            # >>> AJOUT: Initialiser le solde de déficit reportable <<<
            ...
            for idx, year_num in enumerate(years):
                ...
                # --- Calcul P&L ---
                # Utiliser la valeur calculée de la TURPE dans l'EBITDA
                turpe_this_year = turpe_initial
                if turpe_indexed and idx > 0: # Indexation commence année 2 (idx=1)
                    # Utiliser le facteur d'inflation GÉNÉRAL ici, sauf si un taux spécifique est prévu
                    # Assumons inflation générale pour l'instant
                    turpe_this_year *= inflation_factor # Utilise facteur inflation général
                
                # --- AJOUT STOCKAGE ET DEBUG --- 
                # S'assurer que le tableau pour TURPE existe
                if "turpe" not in arrays:
                     arrays["turpe"] = np.zeros(simulation_years)
                arrays["turpe"][idx] = turpe_this_year # Stocker la valeur calculée
                
                # Afficher pour debug
                if idx < 5 or idx == simulation_years - 1: # Afficher pour les premières années et la dernière
                     print(f"DEBUG ENGINE (An {year_num}): turpe_this_year = {turpe_this_year:.2f}") # <-- AJOUTER
                # --- FIN AJOUT --- 

                arrays["ebitda"][idx] = arrays["revenues"][idx] - arrays["opex"][idx] - turpe_this_year # <-- Intégration TURPE ici
                arrays["ebit"][idx] = arrays["ebitda"][idx] - arrays["depreciation"][idx]
                arrays["ebt"][idx] = arrays["ebit"][idx] - arrays["interest_paid"][idx]

                # >>> DEBUT MODIFICATION: Calcul des impôts avec report de déficit <<<
                ebt_this_year = arrays["ebt"][idx]

                # Déterminer le montant du déficit reportable utilisable cette année
                # On ne peut utiliser que jusqu'à couvrir le bénéfice actuel
                loss_used_this_year = min(loss_carryforward_balance, max(0, ebt_this_year))

                # Calculer l'EBT imposable après utilisation du déficit reporté
                taxable_ebt = ebt_this_year - loss_used_this_year

                # Calculer l'impôt sur la base imposable (minimum 0)
                tax_this_year = max(0, taxable_ebt * taux_imposition)
                arrays["taxes"][idx] = tax_this_year

                # Mettre à jour le solde de déficit reportable
                loss_carryforward_balance -= loss_used_this_year # Réduire le solde du montant utilisé
                if ebt_this_year < 0:
                    loss_carryforward_balance += abs(ebt_this_year) # Ajouter la perte de l'année au solde

                # >>> FIN MODIFICATION <<<

                # Calcul du Résultat Net (utilise la nouvelle valeur de taxes)
                arrays["net_income"][idx] = arrays["ebt"][idx] - arrays["taxes"][idx] # <--- Reste inchangé en logique, mais utilise la nouvelle taxe
            ...
            # --- AJOUT PRINT DEBUG --- 
            print(f"DEBUG ENGINE: results['turpe'] avant retour = {results.get('turpe', 'NON TROUVÉ')[:5]}...") # <-- AJOUTER (affiche les 5 premières valeurs)
            # --- FIN AJOUT ---
    ...
    def simulate_selling_price(self,
        ...
        try:
            ...
            if target_npv is not None and abs(target_npv) < 1e-9:
                ...
                wacc = base['wacc']
                # S'assurer que ce sont des arrays numpy pour la vectorisation
                auto = np.array(base.get('annual_autoconsumption', []))
                surplus = np.array(base.get('annual_surplus', []))
                opex = np.array(base.get('opex', []))
                ...
                if wacc is None or len(auto) == 0 or len(surplus) == 0 or len(opex) == 0 or len(auto) != len(surplus) or len(auto) != len(opex):
                     raise ValueError("Données invalides ou vides pour calcul analytique (VAN=0).")
                if abs(wacc - (-1.0)) < 1e-9: raise ValueError("WACC invalide (-100%) pour calcul analytique (VAN=0)")

                N = len(auto)
                try:
                    # --- DEBUT OPTIMISATION NUMPY --- 
                    annees_vec = np.arange(N)          # Vecteur 0, 1, ..., N-1
                    annees_1_N = np.arange(1, N + 1) # Vecteur 1, 2, ..., N
                    
                    dfs_vec = (1 + wacc)**annees_1_N  # Facteurs d'actualisation (vectorisé)
                    if np.any(np.abs(dfs_vec) < 1e-12): raise ValueError("Facteur actualisation nul (VAN=0).")

                    discounted_auto_vec = np.sum(auto / dfs_vec)
                    discounted_opex_vec = np.sum(opex / dfs_vec)
                    
                    # Calcul vectorisé du revenu surplus actualisé
                    oa_factors = (1 + infl_oa)**annees_vec # Facteurs d'indexation OA
                    discounted_surplus_rev_vec = np.sum((surplus * tarif_oa_0 * oa_factors) / dfs_vec)
                    # --- FIN OPTIMISATION NUMPY --- 

                except (OverflowError, ZeroDivisionError, ValueError) as e:
                     # Remplacer l'ancienne exception par celle-ci
                     raise ValueError(f"Erreur actualisation vectorisée (VAN=0): {e}") from e
                
                # Utiliser les résultats vectorisés
                if abs(discounted_auto_vec) < 1e-9:
                    # raise ValueError("Autoconsommation actualisée nulle, calcul prix plancher P impossible.") # <-- Ancienne ligne
                    # --- NOUVELLE GESTION --- 
                    print("ERREUR MOTEUR (VAN=0): Autoconsommation actualisée nulle ou quasi-nulle.")
                    print(" -> Le calcul analytique du Prix Plancher est impossible.")
                    print(" -> Vérifiez la superposition/validité des données de production et de consommation agrégées.")
                    # Lever une erreur spécifique et plus claire (Option 1)
                    raise ValueError("Autoconsommation actualisée nulle, impossible de calculer le Prix Plancher analytiquement. Vérifiez les données d'énergie.")
                    # --- FIN NOUVELLE GESTION --- 
                else:
                    # Calcul normal du prix plancher si l'autoconsommation est non nulle
                    prix_plancher = (capex_net + discounted_opex_vec - discounted_surplus_rev_vec) / discounted_auto_vec
                    optimal_price = prix_plancher
                    # Modifier le message de log
                    print(f"MOTEUR: Prix plancher analytique (vectorisé) calculé: {prix_plancher:.6f}")
