...
# Tarifs TURPE 6 (Fév 2025) pour ACOC Producteur - A placer dans config.py
TURPE_PROD_RATES = {
    "BT<=36kVA": {
    ...
    "HTA": {
        # Tiré de l'exemple Enedis p.10 pour "Autoproducteurs individuels avec injection"
        # C'est une approximation car pas de ligne explicite ACOC HTA, prendre valeur Unique comme base
        "CG": {"Unique": 725.16, "CARD": 725.16}, # Approximation, vérifier si contrat CARD spécifique
        "CC": {"Mensuelle": 383.76}             # p.11
    }
    # Note : CI = 0 pour tous ces cas
}
...
class ConfigModule:
    ...
    def initialize_default_config(self):
        ...
        st.session_state.config = {
            ...
            # --- AJOUT Paramètres TURPE Producteur --- 
            "turpe_prod_tension": "BT<=36kVA", # Niveau de tension du producteur
            "turpe_prod_contrat": "Unique",    # Type de contrat du producteur
            "turpe_prod_calculee": 0.0,     # TURPE annuelle HT calculée
            "turpe_indexe_inflation": True,  # Indexer la TURPE calculée sur l'inflation générale ?
            # "turpe_annuel_initial": 500.0, # <-- ANCIENNE CLE SUPPRIMÉE
            # --- FIN AJOUT TURPE --- 
            ...
            "cout_demantelement_pct": 0.0,    # CORRIGÉ: Coût de démantèlement par défaut à 0
            ...
            "constraint_min_project_irr_pct": 8.0,  # AJOUTÉ: TRI minimum en % (exemple: 8%)
    ...
    def show_ui(self):
        ...
        with tab1:
            ...
            # --- Section TURPE Modifiée --- 
            st.markdown("---")
            st.markdown("#### TURPE Producteur (Calcul Automatique)")

            # 1. Calculer la TURPE (basé sur l'état actuel)
            turpe_prod_calculee = 0.0
            selected_tension_local = config.get("turpe_prod_tension", "BT<=36kVA") # Lire les valeurs actuelles de l'état
            selected_contrat_local = config.get("turpe_prod_contrat", "Unique")
            cg_rate_display = 0.0 # Pour affichage dans l'expander
            cc_rate_display = 0.0 # Pour affichage dans l'expander
            try:
                rates_tension = TURPE_PROD_RATES.get(selected_tension_local)
                if rates_tension:
                    cg_rate_display = rates_tension.get("CG", {}).get(selected_contrat_local, 0.0)
                    cc_keys = list(rates_tension.get("CC", {}).keys())
                    cc_key = cc_keys[0] if cc_keys else None
                    cc_rate_display = rates_tension.get("CC", {}).get(cc_key, 0.0) if cc_key else 0.0
                    turpe_prod_calculee = cg_rate_display + cc_rate_display
                st.session_state.config["turpe_prod_calculee"] = turpe_prod_calculee # Mettre à jour l'état
            except Exception as e:
                # Gérer l'erreur silencieusement ici
                st.session_state.config["turpe_prod_calculee"] = 0.0

            # 2. Afficher directement la valeur calculée (toujours visible)
            st.metric("TURPE Producteur Calculée (€ HT/an)", f"{st.session_state.config.get('turpe_prod_calculee', 0.0):.2f}")

            # 3. Mettre les sélecteurs et la case à cocher dans un expander
            with st.expander("🔧 Modifier les paramètres de calcul TURPE Producteur"):
                col_turpe_exp1, col_turpe_exp2 = st.columns(2)
                with col_turpe_exp1:
                    tension_options = list(TURPE_PROD_RATES.keys())
                    # Lire la valeur depuis l'état pour l'index par défaut
                    current_tension_val = st.session_state.config.get("turpe_prod_tension", "BT<=36kVA")
                    try:
                        current_tension_idx = tension_options.index(current_tension_val)
                    except ValueError: # Gérer si valeur stockée invalide
                         current_tension_idx = 0
                         st.session_state.config["turpe_prod_tension"] = tension_options[0]
                         
                    selected_tension_exp = st.selectbox(
                        "Niveau Tension Raccordement", # Label légèrement raccourci
                        options=tension_options,
                        index=current_tension_idx,
                        key="turpe_tension_select_exp"
                    )
                    # Mettre à jour l'état immédiatement si la sélection change
                    st.session_state.config["turpe_prod_tension"] = selected_tension_exp

                with col_turpe_exp2:
                    contrat_options = ["Unique", "CARD"]
                     # Lire la valeur depuis l'état pour l'index par défaut
                    current_contrat_val = st.session_state.config.get("turpe_prod_contrat", "Unique")
                    try:
                    ...
                    selected_contrat_exp = st.selectbox(
                        "Type Contrat", # Label raccourci
                        options=contrat_options,
                        index=current_contrat_idx,
                        key="turpe_contrat_select_exp",
                        # --- AJOUT DE L'AIDE ICI --- 
                        help=(
                            "Choisir le type de contrat pour l'accès au réseau du producteur :\n\n"
                            "**Contrat Unique :** Contrat entre le producteur et son fournisseur "
                            "d'électricité, couvrant à la fois la fourniture (si applicable) et l'accès au réseau. "
                            "Le fournisseur gère la relation avec Enedis pour l'accès au réseau.\n\n"
                            "**CARD (Contrat d'Accès au Réseau de Distribution) :** Contrat signé "
                            "directement entre le producteur et Enedis, uniquement pour l'accès "
                            "au réseau (injection/soutirage). La fourniture est gérée séparément."
                        )
                        # --- FIN DE L'AJOUT --- 
                    )
                     # Mettre à jour l'état immédiatement si la sélection change
                ...
                # Afficher une note sur la source des tarifs dans l'expander
                st.caption(f"Calcul basé sur TURPE 6 Fév. 2025 (CG={cg_rate_display:.2f}, CC={cc_rate_display:.2f})")

                # Indexation (reste dans l'expander)
                index_turpe_checkbox_exp = st.checkbox(
                    "Indexer la TURPE sur Inflation Générale ?",
                    value=config.get("turpe_indexe_inflation", True),
                    key="turpe_index_check_auto_exp",
                    help="Si coché, le coût TURPE augmentera chaque année avec l'inflation générale du projet."
                )
                st.session_state.config["turpe_indexe_inflation"] = index_turpe_checkbox_exp
            # --- Fin Expander TURPE --- 
            ...
            if st.session_state.config["with_loan"]:
                ...
                # --- AJOUT DES PÉRIODES DE BLOCAGE ICI ---
                st.markdown("---") # Séparateur
                st.markdown("#### Périodes de Blocage")
                col_bloc1, col_bloc2 = st.columns(2)
                with col_bloc1:
                    st.session_state.config["shl_blocage_period"] = st.number_input(
                        "Période de blocage SHL (mois)",
                        min_value=0,
                        max_value=360, # 30 ans max
                        value=int(config.get("shl_blocage_period", 60)), # Lire la config, défaut 60
                        step=12, # Par année
                        help="Durée minimale avant remboursement des prêts actionnaires (SHL).",
                        key="shl_blocage_input" # Clé unique
                    )
                with col_bloc2:
                    st.session_state.config["dividends_blocage_period"] = st.number_input(
                        "Période de blocage dividendes (mois)",
                        min_value=0,
                        max_value=360, # 30 ans max
                        value=int(config.get("dividends_blocage_period", 60)), # Lire la config, défaut 60
                        step=12, # Par année
                        help="Durée minimale avant la distribution des dividendes.",
                        key="dividends_blocage_input" # Clé unique
                    )
                # --- FIN AJOUT PÉRIODES DE BLOCAGE ---
            else:
                # CORRECTION: Utilisation de .get() et float()
        ...
        with tab2:
            ...
            with col1:
                ...
                # Durée PPA (en mois)
                duree_ppa_input = st.number_input(
                    ...
                    key="config_duree_ppa"
                )
                st.session_state.config['duree_ppa'] = duree_ppa_input

                # --- AJOUT DU CHAMP DURÉE CONSTRUCTION --- 
                duree_construction_input = st.number_input(
                    "Durée de construction (mois)",
                    min_value=1, max_value=36, # Ajuster les bornes si nécessaire
                    value=int(config.get('duree_construction', 12)), # Utiliser la valeur existante ou un défaut
                    step=1, 
                    format="%d",
                    key="config_duree_construction",
                    help="Durée estimée de la phase de construction avant le début de l'exploitation."
                )
                st.session_state.config['duree_construction'] = duree_construction_input
                # --- FIN AJOUT --- 
            ...
            with col2:
                ...
                st.session_state.config["cout_demantelement_pct"] = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=100.0, # <-- Augmenter cette valeur
                    value=float(config.get("cout_demantelement_pct", 0.0)),
                    step=1.0,
        ...
        with st.expander("Afficher le récapitulatif des paramètres"):
            # Paramètres économiques
            st.markdown("#### Paramètres Économiques Généraux")
            ...
            st.write(f"Tarif EDF de référence: {st.session_state.config.get('tarif_edf_reference', 0.21):.4f} €/kWh")
            # --- AJOUT Tarif OA --- 
            oa_val = st.session_state.config.get('tarif_oa', 0.0)
            oa_indexe = st.session_state.config.get('tarif_oa_indexe_inflation', True)
            oa_index_txt = "(indexé)" if oa_indexe else "(non indexé)"
            st.write(f"Tarif OA Rachat Surplus: {oa_val:.4f} €/kWh {oa_index_txt}")
            # --- FIN AJOUT ---

            # --- AJOUT Récapitulatif TURPE --- 
            turpe_calc = st.session_state.config.get('turpe_prod_calculee', 0.0)
            turpe_tension = st.session_state.config.get('turpe_prod_tension', 'N/A')
            turpe_contrat = st.session_state.config.get('turpe_prod_contrat', 'N/A')
            turpe_index_txt = "(indexée)" if st.session_state.config.get('turpe_indexe_inflation', True) else "(non indexée)"
            st.write(f"TURPE Producteur ({turpe_tension}, {turpe_contrat}): {turpe_calc:,.2f} € HT/an {turpe_index_txt}")
            # --- FIN AJOUT TURPE --- 
            ...
            if st.session_state.config.get('with_loan', True):
                ...
                st.write(f"Coût des fonds propres: {st.session_state.config.get('cout_fonds_propres', 8.0)}%")
            st.write(f"Période de blocage SHL: {st.session_state.config.get('shl_blocage_period', 60)} mois") # Déjà ajouté ?
            st.write(f"Période de blocage des dividendes: {st.session_state.config.get('dividends_blocage_period', 60)} mois") # Déjà ajouté ?
            
            # Paramètres du projet
            st.markdown("#### Paramètres du Projet")
            st.write(f"Puissance: {st.session_state.config.get('puissance_kwc', 0.0):.1f} kWc") # Ajout Puissance
            st.write(f"CAPEX: {st.session_state.config.get('capex', 0.0):,.2f} €")
            # --- AJOUT Subvention --- 
            subvention_calc = st.session_state.config.get('subvention_calculee', 0.0)
            st.write(f"Subvention Calculée: {subvention_calc:,.2f} €")
            # --- FIN AJOUT ---
            ...
            st.write(f"Date de début du PPA: {st.session_state.config.get('date_debut_ppa', 'N/A')}")
            st.write(f"Durée PPA: {st.session_state.config.get('duree_ppa', 240)} mois") # Ajout Durée PPA
            st.write(f"Taux de dégradation: {st.session_state.config.get('degradation_rate', 0.005)*100:.1f}%/an") # Ajout Dégradation

            # --- AJOUT Amortissement, Résiduel, Démantèlement --- 
            st.write(f"Durée d'amortissement: {st.session_state.config.get('amortissement_duree', 15)} ans")
            val_res_pct = st.session_state.config.get('valeur_residuelle_pct', 0.0) * 100
            st.write(f"Valeur résiduelle: {val_res_pct:.1f}% du CAPEX net")
            cout_dem_pct = st.session_state.config.get('cout_demantelement_pct', 0.0) * 100
            st.write(f"Coût démantèlement: {cout_dem_pct:.1f}% du CAPEX brut")
            # --- FIN AJOUT ---

            # --- AJOUT Source Prix Autoconso --- 
            source_auto_key = st.session_state.config.get("source_prix_autoconso", "prix_initial")
            source_options_map = {
                "prix_initial": "Prix de vente initial",
                "tarif_edf": "Tarif EDF de référence",
                "tarif_oa": "Tarif OA"
            }
            st.write(f"Valorisation autoconsommation: {source_options_map.get(source_auto_key, 'Inconnu')}")
            # --- FIN AJOUT ---
            
            # --- SUPPRESSION de l'ancienne section Contraintes d'Optimisation --- 
    ...
    def ensure_config_defaults(self):
        ...
        default_config_dict = { 
             ...
             "cout_demantelement_pct": 0.0, # MODIFIÉ: Défaut à 0
             "source_prix_autoconso": "prix_initial", # MODIFIÉ: Default = Prix de vente initial
             # Paramètres économiques
             "capex": 85000.0,
             "opex": 4500.0,
             ...
             # Contraintes d'optimisation
             "constraint_min_project_irr_pct": 8.0, # <-- Clé renommée
