...
def display_annual_detailed_summary(results: dict | None, table_map: dict | None, scenario_name_for_key: str = "default_scenario_annual"):
    ...
    # Fonction helper pour obtenir les valeurs annuelles
    def get_annual_total_val(df_source, key_name, default=np.nan):
        if key_name in df_source.columns:
            val = pd.to_numeric(df_source[key_name], errors='coerce').sum(skipna=True)
            return val if pd.notna(val) and np.isfinite(val) else default
        return default

    # Calculer les composantes du compte de résultat
    val_vente_surplus_an = get_annual_total_val(df_year_selected_data, 'Revenus_Surplus')
    val_valorisation_autoconso_an = get_annual_total_val(df_year_selected_data, 'Revenus_Autoconsommation')
    val_prime_autoconso_an = get_annual_total_val(df_year_selected_data, 'Prime_Autoconso_Encaissee')
    
    val_turpe_an = get_annual_total_val(df_year_selected_data, 'TURPE')
    val_maintenance_an = get_annual_total_val(df_year_selected_data, 'OPEX_Maintenance_Mensuel')
    val_assurance_an = get_annual_total_val(df_year_selected_data, 'OPEX_Assurance_Mensuel')
    val_admin_an = get_annual_total_val(df_year_selected_data, 'OPEX_Admin_Mensuel')
    val_provision_onduleur_an = get_annual_total_val(df_year_selected_data, 'OPEX_Provision_Onduleur_Mensuel')
    
    val_amort_an = get_annual_total_val(df_year_selected_data, 'Amortissement')
    val_interets_an = get_annual_total_val(df_year_selected_data, 'Interets_Payes')
    val_total_is_decaisse_an = get_annual_total_val(df_year_selected_data, 'Total_IS_Decaisse_Mois')
