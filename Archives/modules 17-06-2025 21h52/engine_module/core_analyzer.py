...
# Import du système de debug financier
...
class AnalysisEngine:
    ...
    def calculate_net_equity_investment_professional(self, capex_total, debt_amount, subvention_montant):
        ...
        # CORRECTION PROFESSIONNELLE : La subvention réduit l'investissement initial, elle n'est PAS un flux
    ...
    def calculate_fcfe_professional(self, monthly_results_df, subvention_totale_projet):
        ...
        for idx, row in monthly_results_df.iterrows():
            ...
            # NE PAS inclure la prime autoconso dans les FCFE
            prime_encaissee = row.get('Prime_Autoconso_Encaissee', 0)
            if prime_encaissee > 0:
                fcfe_operational -= prime_encaissee  # Retirer la prime des flux
    ...
    def verify_equity_calculations_coherence(self, irr, npv, payback, investment, discount_rate):
        ...
        # Règle 3 : Si payback < durée projet et NPV < 0, problème
    ...
    def calculate_financial_indicators(self,
        ...
        try:
            ...
            # SYSTÈME DE DEBUG POUR TRACER LE PROBLÈME TRI/VAN
            debug_mode = True  # Activer temporairement
            if debug_mode:
                debug_log = {
            ...
            # DEBUG : Tracer le calcul de l'investissement net
            if debug_mode:
            ...
            if fcfe_construction.sum() < -50000:
                logger.warning(f"ALERTE: FCFE construction très négatif ({fcfe_construction.sum():.0f}€) - possible double comptabilisation CAPEX")
            ...
            # DEBUG: Tracer les paramètres WACC pour diagnostiquer NPV=NaN
            logger.info(f"WACC_DEBUG: debt_ratio={debt_ratio_config}, taux_interet_dette={taux_interet_dette_pct_config}, taux_imposition={taux_imposition_standard_pct}, cout_fonds_propres={re_annual}")
            ...
            if wacc_annual_at_pct is None: 
                logger.warning("WACC_DEBUG: WACC calculation returned None, using fallback 6.0%")
            ...
            # CORRECTION CRITIQUE : Les FCFE ne contiennent pas l'investissement initial !
            # Il faut l'ajouter comme premier élément négatif
            if abs(net_equity_investment_total) > 1e-6:  # Si investissement non négligeable
                # L'investissement initial doit être négatif (sortie de cash)
                initial_investment = -abs(net_equity_investment_total)
                equity_cash_flows_for_irr_npv = np.concatenate([[initial_investment], equity_cash_flows_for_irr_npv])
            
            # DEBUG : Tracer les cash flows après correction
            if debug_mode:
            ...
            # DEBUG : Tracer tous les résultats
            if debug_mode:
                ...
                # NOUVEAU DEBUG : Vérifier l'absence de double comptabilisation CAPEX
            ...
            # DEBUG: Tracer les paramètres pour le calcul NPV projet
            logger.info(f"NPV_PROJECT_DEBUG: wacc_monthly_at_rate={wacc_monthly_at_rate}, is_valid={pd.notna(wacc_monthly_at_rate)}")
            logger.info(f"NPV_PROJECT_DEBUG: project_cash_flows length={len(project_cash_flows_for_irr_npv)}, sum={sum(project_cash_flows_for_irr_npv):.2f}")
            logger.info(f"NPV_PROJECT_DEBUG: first_5_cashflows={project_cash_flows_for_irr_npv[:5]}")
            ...
            if pd.isna(npv_project):
                logger.warning("NPV_PROJECT_DEBUG: NPV project calculation returned NaN")
            ...
            # DEBUG : Finaliser les logs et afficher les résultats
            if debug_mode:
                ...
                # Sauvegarder le log
                import json
                with open('debug_equity_calculations.json', 'w') as f:
                    json.dump(debug_log, f, indent=2)
                
                # Afficher dans la console
                print("\n=== DEBUG EQUITY CALCULATIONS ===")
                print(f"Scénario: {scenario_name}")
                print(f"Net Equity Investment: {net_equity_investment_total:,.2f}€")
                print(f"First 5 cashflows: {debug_log['calculs_equity']['cashflows_24_months'][:5]}")
                print(f"IRR calculated: {irr_equity:.2%}" if pd.notna(irr_equity) else f"IRR: {irr_equity}")
                print(f"IRR method: {irr_calculation_method}")
                print(f"IRR status: {irr_equity_warning}")
                print(f"NPV calculated: {npv_equity:,.2f}€" if pd.notna(npv_equity) else f"NPV: {npv_equity}")
                print(f"NPV manual check: {debug_log['calculs_equity'].get('npv_manual_check', 'N/A')}")
                print(f"Discount rate (monthly): {re_monthly_rate:.6f}" if pd.notna(re_monthly_rate) else f"Discount rate: {re_monthly_rate}")
                print(f"Payback equity: {payback_equity_years:.1f} ans" if pd.notna(payback_equity_years) else f"Payback: {payback_equity_years}")
                
                # Vérification double comptabilisation
                fcfe_construction_sum = debug_log['calculs_equity'].get('fcfe_construction_sum', 0)
                print(f"FCFE construction sum: {fcfe_construction_sum:,.0f}€")
                if debug_log['calculs_equity'].get('double_capex_check', False):
                    print("⚠️ DOUBLE COMPTABILISATION CAPEX DÉTECTÉE")
                else:
                    print("✅ Pas de double comptabilisation CAPEX")
                
                if coherence_errors:
                    print("\n!!! ERREURS DE COHÉRENCE DÉTECTÉES !!!")
                    for error in coherence_errors:
                        print(f"  - {error}")
                else:
                    print("✅ Cohérence mathématique OK")
