...
def simulate_lcoe_for_project_sizes(
    ...
    for size_kwc_iter in project_sizes_kwc_list:
        ...
        if 'production_kwh' in scaled_profile_df.columns:
            ...
            results_list_lcoe.append({'Taille_kWc': size_kwc_iter, 'LCOE': np.nan, 'CAPEX_Total_Utilise_Simulation': current_capex_val, 'OPEX_Total_Annuel_Simule': np.nan, 'TURPE_Annuel_Simule': np.nan, 'Taux_Autoconsommation_Simule': np.nan, 'Taux_Autoproduction_Simule': np.nan})
            continue
        ...
        # Le prix de revente a peu d'impact sur le LCOE lui-même.
        prix_revente_sim_lcoe = current_sim_engine_config.get("prix_vente_initial_slider_fallback", 0.10)

        indicators_lcoe_dict = current_sim_engine.calculate_financial_indicators(
            scenario_name=target_scenario_name_sim,
            prix_revente=prix_revente_sim_lcoe,
            sites_config=current_iter_sites_config_dict # Config pour le site simulé
        )

        # 6. Stocker les résultats
        if indicators_lcoe_dict and not indicators_lcoe_dict.get("error"):
            lcoe_val_iter = indicators_lcoe_dict.get('lcoe')
            ...
            monthly_df_sim_res = indicators_lcoe_dict.get('monthly_data')
            ...
            results_list_lcoe.append({
                ...
                'CAPEX_Total_Utilise_Simulation': indicators_lcoe_dict.get('capex_scenario_final_utilise', current_capex_val),
            ...
            error_detail = indicators_lcoe_dict.get("error", "inconnue") if isinstance(indicators_lcoe_dict, dict) else "calcul non concluant"
...
def create_lcoe_vs_size_chart(df_lcoe_results: pd.DataFrame, lcoe_target: float | None = None):
    ...
    fig.update_layout(
        title_text="LCOE en fonction de la Taille du Projet (basé sur profil agrégé du projet actuel)",
        xaxis_title="Taille du Projet Simulé (kWc)", yaxis_title="LCOE (€/kWh)",
        yaxis_tickformat=".4f", hovermode="x unified",
...
def create_autoconsumption_vs_size_chart(df_results: pd.DataFrame):
    ...
    fig.update_layout(
        title_text="Taux d'Autoconsommation & Autoproduction vs Taille",
...
def display_lcoe_analysis_section(analysis_engine, config, sites_config, scenarios):
    ...
    if st.button("Lancer l'Analyse LCOE par Taille", key="run_lcoe_analysis_button_ui_v5", type="primary"):
        if not project_sizes_to_test:
            ...
            with st.spinner("Préparation du profil de référence et simulation des LCOE..."):
                ...
                if base_aggregated_profile_df is None or original_total_power_ref <=0 :
                     ...
                     st.session_state['df_lcoe_analysis_results'] = pd.DataFrame() # Assurer un état propre
                else:
                    ...
                    st.session_state['df_lcoe_analysis_results'] = df_lcoe_data
            st.rerun()

    if 'df_lcoe_analysis_results' in st.session_state:
        df_results_lcoe_ui = st.session_state.df_lcoe_analysis_results
        if not df_results_lcoe_ui.empty:
            st.markdown("---")
            st.markdown("#### Résultats de l'Analyse LCOE et Autoconsommation par Taille")
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Taux_Autoconsommation_Simule": st.column_config.NumberColumn("Taux Autoconsommation Simulé (%)", format="%.1f %%"),
                    "Taux_Autoproduction_Simule": st.column_config.NumberColumn("Taux Autoproduction Simulé (%)", format="%.1f %%"),
                },
                use_container_width=True,
                hide_index=True
            )

            # Disposition des graphiques côte à côte dans deux colonnes
            col_graph1, col_graph2 = st.columns(2)
            
            with col_graph1:
                fig_lcoe_ui = create_lcoe_vs_size_chart(df_results_lcoe_ui, lcoe_target_input if lcoe_target_input > 0 else None)
                if fig_lcoe_ui:
                    st.plotly_chart(fig_lcoe_ui, use_container_width=True)
            
            with col_graph2:
                fig_autoconsumption_ui = create_autoconsumption_vs_size_chart(df_results_lcoe_ui)
                if fig_autoconsumption_ui:
                    st.plotly_chart(fig_autoconsumption_ui, use_container_width=True)
        elif 'df_lcoe_analysis_results' in st.session_state: # Clé existe mais DF est vide
