...
class AnalysisEngine:
    ...
    def calculate_financial_indicators(self,
        ...
        try:
            ...
            EQUITY_INVESTMENT_THRESHOLD = 1.0  # Par exemple, 1 Euro
            ...
            # S'assurer que ebit_mensuel_series est défini
            # L'EBIT est calculé dans la boucle mensuelle et stocké dans monthly_results_df['EBIT']
            ebit_mensuel_series = monthly_results_df['EBIT'].fillna(0)

            # Calcul du NOPAT (Net Operating Profit After Tax)
            # NOPAT = EBIT * (1 - Taux d'imposition statutaire)
            nopat_mensuel_series = ebit_mensuel_series * (1.0 - tax_rate_decimal)

            # Calcul de OCF_Projet (FCFF) basé sur NOPAT
            # FCFF = NOPAT + Amortissement - CAPEX Net - ΔBFR - Paiement Net de TVA
            # Note: 'amortissement_mensuel' est la série des amortissements mensuels.
            #       'capex_net_mensuel_series' est la série des investissements nets mensuels.
            #       'delta_bfr_mensuel_series' est la série de la variation du BFR mensuel.
            #       'vat_payment_mensuel' est la série des paiements nets de TVA mensuels.
            
            monthly_results_df['OCF_Projet'] = (nopat_mensuel_series + \
                                               amortissement_mensuel + \
                                               - capex_net_mensuel_series + \
                                               - delta_bfr_mensuel_series + \
                                               - vat_payment_mensuel)
