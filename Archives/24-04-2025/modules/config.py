    ...
    def show_ui(self):
        ...
        with tab2:
            ...
            with col1:
                ...
                # Durée Construction (en mois)
                st.session_state.config["duree_construction"] = st.number_input(
                    "Durée Construction (mois)",  # Le label qui s'affichera
                    min_value=0,                  # Valeur minimale
                    max_value=36,                 # Valeur maximale (ajustez si besoin)
                    value=config.get("duree_construction", 12), # Lire la valeur actuelle ou 12 par défaut
                    step=1,                       # Incrément
                    help="Durée estimée de la phase de construction en mois (actuellement non utilisée dans les calculs financiers)", # Texte d'aide
                    key="duree_construction_input" # Clé unique pour ce widget
                )
