...
class DataImportModule:
    ...
    def show_ui(self):
        ...
        st.markdown("<h3 class='sub-header'>Importation Fichier PVSOL</h3>", unsafe_allow_html=True)
        
        # Uploader pour le fichier PVSOL
        pvsol_file = st.file_uploader(
            "Importer le fichier Excel de PVSOL",
            type=['xlsx', 'xls'],
            key="pvsol_excel_uploader"
        )
        
        if pvsol_file is not None:
            try:
                # Lire le contenu du fichier en mémoire
                file_content = pvsol_file.read()
                ...
                st.success(f"Fichier {pvsol_file.name} chargé avec succès.")
                ...
                # Éviter les doublons - s'assurer que tous les index sont différents
                if prod_index == date_index and len(available_columns) > 1:
                    candidates = [i for i in range(len(available_columns)) if i != date_index]
                    if candidates:
                        prod_index = candidates[0]

                if cons_index == date_index or cons_index == prod_index:
                    if len(available_columns) > 2:
                        candidates = [i for i in range(len(available_columns)) 
                                   if i != date_index and i != prod_index]
                        if candidates:
                            cons_index = candidates[0]

                # Fallback aux index séquentiels si nécessaire
                if date_index is None:
                    date_index = 0
                if prod_index is None:
                    prod_index = min(1, len(available_columns)-1) if len(available_columns) > 1 else 0
                if cons_index is None:
                    cons_index = min(2, len(available_columns)-1) if len(available_columns) > 2 else prod_index

                # Vérification finale pour éviter les index hors limites
                ...
                # Créer les sélecteurs de colonnes avec les index choisis
                ...
                if date_col is not None and prod_col is not None and cons_col is not None:
                    if st.button("Traiter les données", key="process_single_file"):
                        with st.spinner("Traitement des données en cours..."):
                            try:
                                # Stocker le mapping des colonnes dans la session
                                ...
                                processed_df = pd.DataFrame({
                                    'Temps': df_pvsol[date_col],
                                    'production_kwh': df_pvsol[prod_col],
                                    'consumption_kwh': df_pvsol[cons_col]
                                })
                                ...
                                processed_df['consumption_kwh'] = pd.to_numeric(processed_df['consumption_kwh'], errors='coerce')
                                
                                # Calculer l'autoconsommation
                                processed_df['autoconsumption_kwh'] = processed_df.apply(
                                    lambda row: min(row['production_kwh'], row['consumption_kwh']), axis=1
                                )
                    ...
                    st.error("Impossible de traiter les données : certaines colonnes requises n'ont pas été trouvées dans le fichier.")
                    st.info("Veuillez importer un fichier contenant les colonnes 'Temps', 'Énergie PV (CC)' et 'Consommation'.")
