...
# import traceback # Maintenu si utilisé ailleurs, sinon peut être enlevé si non requis par le reste du fichier
...
try:
    ...
    # NOUVEAUX IMPORTS
    from modules.financial_display_utils import load_table_map 
    from modules.annual_summary_display import display_project_summary, display_annual_detailed_summary
    from modules.monthly_cash_flow_display import display_cash_flow_statement_restructured
        ...
        with left_col_tab1:
            ...
            if isinstance(optim_data, dict):
                 ...
                 if 'error' in optim_data:
                     ...
                     prix_optimal_ht_trouve = optim_data['prix_optimal_const']
                     ...
                     # Calcul du Gain Client TTC réel basé sur le prix optimal TTC
                     gain_client_ttc_reel_pct = 0.0
                     if edf_ref_ttc_for_gain_calc > 1e-6 : # Éviter division par zéro
                     ...
                     dscr_res = indic_at_optimal.get('avg_dscr')
                     ...
                     with col_eq1:
                         ...
                         st.metric("TRI Fonds Propres (%)", f"{irr_eq*100:.1f}%" if irr_eq is not None else "N/A",
                                    help="Taux de Rentabilité Interne sur les fonds propres investis.") # Delta retiré car la cible TRI Projet n'est pas celle des FP.
                     with col_eq2:
                          st.metric("Payback Fonds Propres (ans)", f"{payback_eq:.1f}" if payback_eq is not None else "N/A",
                                     delta=f"{(payback_eq - payback_equity_cible_disp):.1f} vs cible" if payback_eq is not None and payback_equity_cible_disp != 'N/A' else None,
                     ...
                     st.markdown("---")
                     st.markdown("**Indicateurs Projet (avant financement, après IS)**")
                     ...
                     with col_proj_res3: # Affichage du Gain Client TTC Réel
                         st.metric("Gain Client Réel (vs EDF TTC)", f"{gain_client_ttc_reel_pct:.1f}%",
                                   delta=f"{gain_client_ttc_reel_pct - (gain_client_cible_ttc_disp if gain_client_cible_ttc_disp != 'N/A' else 0):.1f}% vs cible", 
                                   delta_color="normal",
                        ...
                        try:
                            ...
                            if indic_for_graph and indic_for_graph.get('monthly_data') is not None:
                                ...
                                if 'Autoconsommation_kWh' in df_monthly_graph:
                                     ...
                                     unique_years_graph = df_monthly_graph['Sim_Year'].unique()
                                     if len(unique_years_graph) > 0:
                                         param_autoconso_annuel_graph = df_monthly_graph.groupby('Sim_Year')['Autoconsommation_kWh'].sum().mean()
