...
def validate_and_prepare_sites_data(sites_data_input: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    if not isinstance(sites_data_input, dict):
        logger.error("sites_data_input n'est pas un dictionnaire.")
        return prepared_sites_data # Retourne un dict vide
    ...
    for site_id, df_site_init in sites_data_input.items():
        ...
        if not df_copy.empty:
            if 'Temps' not in df_copy.columns:
                ...
                continue # Ignorer ce site s'il manque 'Temps'
            try:
                if not pd.api.types.is_datetime64_any_dtype(df_copy['Temps']):
                    logger.info(f"Site '{site_id}' - Conversion de 'Temps' en datetime")
                    df_copy['Temps'] = pd.to_datetime(df_copy['Temps'], errors='raise')
                if df_copy['Temps'].isnull().any():
                    logger.error(f"Conversion de 'Temps' en datetime a produit des NaT pour le site '{site_id}'. Ce site sera ignoré.")
                    continue
            except Exception as e_time: 
                logger.error(f"Format de la colonne 'Temps' invalide pour le site '{site_id}': {e_time}. Ce site sera ignoré.")
                continue
            ...
            for col in ['production_kwh', 'consumption_kwh']:
                ...
                try:
                    ...
                    df_copy[col] = 0.0
            prepared_sites_data[site_id] = df_copy
            logger.info(f"Site '{site_id}' - Ajouté aux données préparées avec shape: {df_copy.shape}")
        else:
            prepared_sites_data[site_id] = df_copy # Stocker le DataFrame vide s'il l'était initialement
            logger.info(f"Données vides pour le site '{site_id}'.")
    
    logger.info(f"FIN validate_and_prepare_sites_data - Nombre de sites préparés: {len(prepared_sites_data)}")
    for site_id, df in prepared_sites_data.items():
