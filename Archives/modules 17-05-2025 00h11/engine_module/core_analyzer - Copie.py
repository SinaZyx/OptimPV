        ...
        try:
            ...
            for month_idx in range(num_total_simulation_months):
                ...
                if is_construction_phase:
                    ...
                    try:
                        production_values = hourly_data_to_return.loc[hourly_data_to_return.index.year == current_month_end_date.year]['production'].values
                        consumption_values = hourly_data_to_return.loc[hourly_data_to_return.index.year == current_month_end_date.year]['consumption'].values
                        
                        # Log des sommes des arrays numpy
                        logger.info(f"CORE DEBUG DANS TRY - Mois {current_month_end_date.month} - production_values.sum(): {production_values.sum()}")
                        logger.info(f"CORE DEBUG DANS TRY - Mois {current_month_end_date.month} - consumption_values.sum(): {consumption_values.sum()}")

                        autoconsommation_horaire_np = np.minimum(production_values, consumption_values)
                        logger.info(f"CORE DEBUG DANS TRY - Mois {current_month_end_date.month} - autoconsommation_horaire_np.sum(): {autoconsommation_horaire_np.sum()}")

                        surplus_horaire_np = production_values - autoconsommation_horaire_np
                        surplus_horaire_np = np.maximum(0, surplus_horaire_np)
                        logger.info(f"CORE DEBUG DANS TRY - Mois {current_month_end_date.month} - surplus_horaire_np.sum(): {surplus_horaire_np.sum()}")
                        ...
                        logger.info(f"CORE DEBUG DANS TRY - Mois {current_month_end_date.month} - prod_m_calc: {prod_m_calc:.2f}, cons_m_calc: {cons_m_calc:.2f}, auto_m_calc: {auto_m_calc:.2f}, surplus_m_calc: {surplus_m_calc:.2f}")

                    except Exception as e_autoc:
                        logger.error(f"ERREUR DANS CALCUL AUTOCONSOMMATION MENSUELLE - Mois {current_month_end_date.month}: {e_autoc}")
                        logger.error(traceback.format_exc())
                    
                    # Assignation aux variables prod_m, cons_m utilisées ensuite
                    prod_m = prod_m_calc
                    cons_m = cons_m_calc
                    auto_m = auto_m_calc
                    ...
                    # Vérification supplémentaire juste avant le log problématique
