...
def display_annual_detailed_summary(results: dict | None, table_map: dict | None, scenario_name_for_key: str = "default_scenario_annual"):
    ...
    # Calculer les produits financiers
    val_interets_placements_an = calculate_annual_total_from_monthly(monthly_df_raw, 'Interets_Placements_Mensuels', selected_year_for_display)
    ...
    try:
        ...
        log_tva_placement(f"COMPTE DE RÉSULTAT - ANNÉE {selected_year_for_display}", {
            "Intérêts_placements_annuels": f"{val_interets_placements_an:,.2f}€" if val_interets_placements_an else "0.00€",
    ...
    # Modifier le calcul du total_produits_exploitation pour inclure les produits financiers
    total_produits_avec_financiers = total_produits_exploitation + (val_interets_placements_an or 0)
    ...
    resultat_avant_impot = resultat_exploitation - (val_interets_an or 0) + (val_interets_placements_an or 0)
    ...
    compte_resultat_data = [
        ...
        # PRODUITS FINANCIERS (après CHARGES FINANCIÈRES)
        {
            'type': 'section_header',
            'label': 'PRODUITS FINANCIERS',
            'value': val_interets_placements_an,
            'percentage': format_percentage(val_interets_placements_an, total_produits_exploitation),
            'indent': 0
        },
        {
            'type': 'detail',
            'label': 'Intérêts sur placements',
            'value': val_interets_placements_an,
            'percentage': format_percentage(val_interets_placements_an, total_produits_exploitation),
            'indent': 1
        },
