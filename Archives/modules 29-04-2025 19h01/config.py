...
class ConfigModule:
    ...
    def initialize_default_scenarios(self):
        ...
        st.session_state.scenarios = {
            "Base": {"description": "Scénario de référence", "production_modifier": 1.0, "opex_modifier": 1.0, "capex_modifier": 1.0, "inflation_modifier": 1.0,},
    ...
    def show_ui(self):
        ...
        tab_titles = [
            ...
            "Scénarios",                 # Onglet 5
            "Sauvegarde/Chargement"      # Onglet 6
        ]
        tab_eco, tab_proj, tab_fin, tab_sites, tab_scen, tab_save = st.tabs(tab_titles)
        ...
        with tab_eco:
            ...
            with col1:
                ...
                st.session_state.config["taux_imposition"] = st.number_input("Taux d'imposition (%)", min_value=0.0, max_value=50.0, value=float(config.get("taux_imposition", 25.0)), step=0.1, help="Taux d'imposition des bénéfices")
        ...
        with tab_proj:
            ...
            with st.expander("🔧 Configuration Barèmes Subvention Globale"):
                 ...
                 st.session_state.config["subvention_rate_le9"] = st.number_input("Taux si P <= 9 kWc (€/kWc)", value=float(config.get("subvention_rate_le9", 80.0)))
                 # ... autres paliers
            ...
            # ... (logique pour déterminer applicable_rate basé sur total_power_for_sub) ...
        ...
        with tab_fin:
            ...
            if st.session_state.config["with_loan"]:
                 ...
                 with col_f1:
                      ...
                      st.session_state.config["debt_term_years"] = st.number_input("Durée Prêt (ans)", min_value=1, value=int(config.get("debt_term_years", 20)), step=1, key="fin_debt_term")
                 with col_f2:
            ...
            if sites_data_dict:
                ...
                for site_id in site_ids:
                    ...
                    with st.expander(f"Site : {original_filename} (ID: `{site_id}`)"):
                        ...
                        # -- OPEX --
                        with col_s3:
                            opex_options = ["auto", "percent", "kwc", "custom"]
                            opex_labels = {"auto": "Automatique", "percent": "% CAPEX", "kwc": "€/kWc/an", "custom": "Montant Fixe (€/an)"}
                            # Lire méthode existante ou défaut global, forcer custom si conso
                            current_opex_method_from_cfg = site_cfg.get("opex_method", config.get("opex_method", "auto"))
                            # S'assurer que la méthode est valide
                            if current_opex_method_from_cfg not in opex_options: current_opex_method_from_cfg = "auto"
                            # Forcer à 'custom' (et donc 0) si consommateur
                            effective_opex_method = 'custom' if is_consumer else current_opex_method_from_cfg

                            site_opex_method_input = st.selectbox(
                                f"Méthode OPEX",
                                options=opex_options,
                                format_func=lambda x: opex_labels[x],
                                index=opex_options.index(effective_opex_method),
                                key=f"cfg_opex_method_{site_id}",
                                help="Méthode de calcul des OPEX annuels pour ce site. Grisé si 'Consommateur Pur'.",
                                disabled=is_consumer # Griser selectbox si conso
                            )
                            # Sauvegarder la méthode sélectionnée si producteur, sinon 'custom'
                            site_cfg['opex_method'] = 'custom' if is_consumer else site_opex_method_input

                            site_opex_value = 0.0 # Sera la valeur à stocker (initialisée à 0)
                            current_opex_value_from_cfg = site_cfg.get('opex_value', 0.0) # Lire valeur stockée pour ce site

                            # Widget de valeur OPEX: Afficher seulement si producteur ET méthode != auto
                            if not is_consumer and site_cfg['opex_method'] != "auto":
                                # Déterminer la valeur par défaut du widget basé sur la méthode sélectionnée (pour les producteurs)
                                default_widget_value = 0.0
                                if site_cfg['opex_method'] == "percent": default_widget_value = config.get("opex_percent", 1.5)
                                elif site_cfg['opex_method'] == "kwc": default_widget_value = config.get("opex_per_kwc", 20.0)
                                elif site_cfg['opex_method'] == "custom": default_widget_value = 0.0 # Pourrait aussi être lu depuis current_opex_value_from_cfg

                                # Pré-remplir avec valeur existante SI elle correspond à la méthode, sinon défaut global
                                final_value_candidate = float(current_opex_value_from_cfg if site_cfg.get("opex_method") == site_cfg['opex_method'] else default_widget_value)

                                # Afficher le widget approprié
                                if site_cfg['opex_method'] == "percent":
                                    min_val_percent = 0.1
                                    value_to_display = max(min_val_percent, final_value_candidate)
                                    site_opex_value = st.number_input(f"OPEX (% CAPEX)", min_value=min_val_percent, value=value_to_display, step=0.1, key=f"cfg_opex_val_{site_id}")
                                elif site_cfg['opex_method'] == "kwc":
                                    min_val_kwc = 1.0
                                    value_to_display = max(min_val_kwc, final_value_candidate)
                                    site_opex_value = st.number_input(f"OPEX (€/kWc/an)", min_value=min_val_kwc, value=value_to_display, step=1.0, key=f"cfg_opex_val_{site_id}")
                                elif site_cfg['opex_method'] == "custom":
                                    min_val_custom = 0.0
                                    value_to_display = max(min_val_custom, final_value_candidate)
                                    site_opex_value = st.number_input(f"OPEX (€/an)", min_value=min_val_custom, value=value_to_display, step=10.0, key=f"cfg_opex_val_{site_id}")

                            # Sauvegarder la valeur: 0 si conso ou auto, sinon la valeur saisie
                            site_cfg['opex_value'] = 0.0 if is_consumer or site_cfg['opex_method'] == "auto" else site_opex_value

                        # --- Provision Onduleur par Site (ADAPTÉ) ---\n                        # Désactiver toute la section si consommateur
                        ...
                        with prov_cols[1]:
                            default_cost = 130.0 # Default cost if global key missing
                            current_cost = float(site_cfg.get("opex_onduleur_cost_site", default_cost))
                            value_cost_widget = max(0.0, current_cost)
                            site_cost_input = st.number_input("Coût (€/kWc)",
                                                        min_value=0.0,
                                                        value=value_cost_widget,
                                                        step=5.0,
                                                        key=f"cfg_prov_cost_{site_id}",
                                                        disabled=not effective_prov_enabled) # Désactiver si checkbox décochée OU si conso
                            # Sauvegarder la valeur si activé (et producteur implicitement), sinon 0
                            site_cfg['opex_onduleur_cost_site'] = site_cost_input if effective_prov_enabled else 0.0
        ...
        # --- Onglet 6: Sauvegarde/Chargement ---
        with tab_save:
    ...
    def ensure_config_defaults(self):
        ...
        default_config_dict = {
             ...
             "dividends_blocage_period": 60, "opex_method": "auto", "opex_percent": 1.5,
             "opex_per_kwc": 20.0,
