...
class AnalysisEngine:
    ...
    def calculate_financial_indicators(self,
        ...
        try:
            ...
            total_opex_base = 0.0 # OPEX sans provision onduleur
            total_puissance_kwc = 0.0
            inverter_configs = [] # Pour stocker les détails de provision par site
            ...
            if sites_config and isinstance(sites_config, dict) and sites_config:
                ...
                opex_percent_global = float(global_config.get('opex_percent', 1.5)) / 100.0
                opex_kwc_global = float(global_config.get('opex_per_kwc', 20.0))

                for site_id, site_cfg in sites_config.items():
                    if not isinstance(site_cfg, dict): continue # Ignorer config site invalide

                    site_power = float(site_cfg.get('puissance_kwc', 0.0))
                    site_capex = float(site_cfg.get('capex', 0.0))
                    opex_method = site_cfg.get('opex_method', 'auto')
                    opex_value_input = site_cfg.get('opex_value', 0.0)

                    # Lire config onduleur spécifique au site
                    site_prov_enabled = site_cfg.get("opex_onduleur_provision_site", False)
                    site_prov_cost_kwc = float(site_cfg.get("opex_onduleur_cost_site", 0.0))
                    ...
                    # Calcul OPEX annuel BASE (sans provision) pour ce site
                    site_opex_annual_base = 0.0
                    if site_power > 0: # Seulement si producteur
                        if opex_method == 'auto':
                            capex_seuil_pour_opex_kwc = 5000 # Seuil exemple
                            if site_capex >= capex_seuil_pour_opex_kwc: site_opex_annual_base = site_capex * opex_percent_global
                            else: site_opex_annual_base = site_power * opex_kwc_global
                        elif opex_method == 'percent': site_opex_annual_base = site_capex * (float(opex_value_input) / 100.0)
                        elif opex_method == 'kwc': site_opex_annual_base = site_power * float(opex_value_input)
                        elif opex_method == 'custom': site_opex_annual_base = float(opex_value_input)
                    total_opex_base += site_opex_annual_base # Somme des OPEX base

                    # Stocker la config onduleur si activée et valide
                    if site_prov_enabled and site_power > 0 and site_prov_cost_kwc > 0 and site_prov_lifetime > 0:
                        inverter_configs.append({
                            "power_kwc": site_power,
                            "cost_per_kwc": site_prov_cost_kwc,
                            "lifetime_years": site_prov_lifetime
                        })
                        # print(f"  -> Site {site_id}: Provision onduleur activée (Coût={site_prov_cost_kwc}€/kWc, Vie={site_prov_lifetime}ans)") # Décommenter si besoin

                print(f"MOTEUR: Totaux Agrégés: CAPEX={total_capex_base:,.2f}€, OPEX_base={total_opex_base:,.2f}€/an, P_totale={total_puissance_kwc:.2f}kWc")
                # Assigner les valeurs agrégées pour la suite
                capex_base = total_capex_base
                opex_annual_base_config = total_opex_base # OPEX base agrégé
                puissance_kwc_global = total_puissance_kwc

            else: # Utilisation config globale (fallback ou si sites_config vide)
                print("MOTEUR: Utilisation de la configuration globale (pas de sites_config ou vide).")
                source_des_inputs = "globaux"
                # --- Utilisation de .get() avec défaut 0.0 ---
                capex_base = float(global_config.get("capex", 0.0))
                opex_annual_base_config = float(global_config.get("opex", 0.0))
                puissance_kwc_global = float(global_config.get("puissance_kwc", 0.0))
                # --- Fin Utilisation de .get() ---
                if capex_base == 0 or puissance_kwc_global == 0:
                     print("AVERTISSEMENT MOTEUR: Valeurs CAPEX/Puissance globales utilisées sont nulles en mode fallback.")
                # La provision onduleur globale n'est plus gérée.
            ...
            for month_idx in range(num_months_simulation):
                ...
                for inv_cfg in inverter_configs: # Utilise la liste créée plus haut
                    if simulation_year > 0 and inv_cfg["lifetime_years"] > 0 and simulation_year % inv_cfg["lifetime_years"] == 0:
                        cost_this_replacement = inv_cfg["power_kwc"] * inv_cfg["cost_per_kwc"]
