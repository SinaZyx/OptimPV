...
try:
    ...
    print("Matplotlib non disponible - utilisation de Plotly uniquement")
...
class DataImportModule:
        ...
        try:
            ...
            if file_extension == 'csv':
                ...
                try:
                    ...
                    # DEBUG: Afficher les premières lignes et colonnes du fichier
                    st.info(f"IMPORT DEBUG - CSV lu avec sep=';' (premiers enregistrements):\n{df.head()}")
                    st.info(f"IMPORT DEBUG - Colonnes détectées: {df.columns.tolist()}")
                    st.info(f"IMPORT DEBUG - Nombre de lignes dans le fichier: {len(df)}")
                except Exception as e1:
                    st.warning(f"Échec lecture CSV avec sep=';' : {str(e1)}")
                    try:
                        ...
                        st.warning(f"Échec lecture CSV avec sep=',' : {str(e2)}")
                        try:
                            # Essai avec détection automatique du séparateur
                            df = pd.read_csv(file, sep=None, engine='python')
                            st.info(f"IMPORT DEBUG - CSV lu avec détection auto (premiers enregistrements):\n{df.head()}")
                            st.info(f"IMPORT DEBUG - Colonnes détectées: {df.columns.tolist()}")
                        except Exception as e3:
                            st.error(f"Impossible de lire le CSV avec aucun séparateur: {str(e3)}")
                            return None
            elif file_extension in ['xls', 'xlsx']:
                df = pd.read_excel(file, skiprows=18)
                # DEBUG: Afficher les premières lignes et colonnes du fichier Excel
                st.info(f"IMPORT DEBUG - Excel lu avec skiprows=18 (premiers enregistrements):\n{df.head()}")
                st.info(f"IMPORT DEBUG - Colonnes détectées: {df.columns.tolist()}")
                st.info(f"IMPORT DEBUG - Nombre de lignes dans le fichier: {len(df)}")
            else:
            ...
            # DEBUG: Vérifier le DataFrame après traitement
            if df is not None:
                st.info(f"IMPORT DEBUG - DataFrame après traitement (premiers enregistrements):\n{df.head()}")
                st.info(f"IMPORT DEBUG - Colonnes après traitement: {df.columns.tolist()}")
                
                if 'production_kwh' in df.columns:
                    production_sum = df['production_kwh'].sum()
                    st.info(f"IMPORT DEBUG - Somme production_kwh: {production_sum:.2f}")
                    st.info(f"IMPORT DEBUG - Nb valeurs non-nulles dans production_kwh: {df['production_kwh'].count()}")
                    st.info(f"IMPORT DEBUG - Min/Max production_kwh: {df['production_kwh'].min():.2f} / {df['production_kwh'].max():.2f}")
                    
                    # Alerte si production quasi nulle
                    if production_sum < 1.0:
                        st.error(f"⚠️ ALERTE: Production totale quasi nulle ({production_sum:.2f})!")
                
                if 'consumption_kwh' in df.columns:
                    consumption_sum = df['consumption_kwh'].sum()
                    st.info(f"IMPORT DEBUG - Somme consumption_kwh: {consumption_sum:.2f}")
                    st.info(f"IMPORT DEBUG - Nb valeurs non-nulles dans consumption_kwh: {df['consumption_kwh'].count()}")
    ...
    def process_production_data(self, df):
        ...
        """
        # DEBUG: Afficher les informations de débogage pour le DataFrame brut
        st.info(f"PROCESS_PROD DEBUG - DataFrame brut (colonnes): {df.columns.tolist()}")
        st.info(f"PROCESS_PROD DEBUG - DataFrame brut (premiers enregistrements):\n{df.head()}")
        ...
        st.info(f"PROCESS_PROD DEBUG - Colonnes de production détectées: {production_cols}")
        st.info(f"PROCESS_PROD DEBUG - Colonnes de date détectées: {date_cols}")
        ...
        if production_cols and date_cols:
            ...
            st.info(f"PROCESS_PROD DEBUG - Colonne date sélectionnée: '{date_col}'")
            st.info(f"PROCESS_PROD DEBUG - Colonne production sélectionnée: '{production_col}'")
            
            # Afficher un échantillon des données des colonnes sélectionnées
            st.info(f"PROCESS_PROD DEBUG - Échantillon colonne date: {df[date_col].head().tolist()}")
            st.info(f"PROCESS_PROD DEBUG - Échantillon colonne production: {df[production_col].head().tolist()}")
            
            # Vérifier si la colonne de production est numérique
            try:
                numeric_values = pd.to_numeric(df[production_col].astype(str).str.replace(',', '.'))
                st.info(f"PROCESS_PROD DEBUG - Somme brute colonne production: {numeric_values.sum():.2f}")
                if numeric_values.sum() < 1.0:
                    st.error(f"⚠️ ALERTE: Production brute quasi nulle ({numeric_values.sum():.2f}) dans la colonne '{production_col}'!")
            except Exception as e:
                st.warning(f"PROCESS_PROD DEBUG - Impossible de convertir la colonne production en numérique: {str(e)}")
            ...
            try:
                ...
                st.info(f"PROCESS_PROD DEBUG - Conversion date réussie. Premier élément: {result_df['date'].iloc[0]}")
            except Exception as e:
                # Si la conversion échoue, conserver la colonne telle quelle
                st.warning(f"PROCESS_PROD DEBUG - Échec conversion date: {str(e)}")
                pass
            ...
            try:
                ...
                st.info(f"PROCESS_PROD DEBUG - Conversion production réussie. Somme: {result_df['production_kwh'].sum():.2f}")
            except Exception as e:
                # Si la conversion échoue, conserver la colonne telle quelle
                st.warning(f"PROCESS_PROD DEBUG - Échec conversion production: {str(e)}")
                pass
            
            # DEBUG: Vérifier le résultat final
            st.info(f"PROCESS_PROD DEBUG - DataFrame résultat (premiers enregistrements):\n{result_df.head()}")
            st.info(f"PROCESS_PROD DEBUG - Somme finale production_kwh: {result_df['production_kwh'].sum():.2f}")
