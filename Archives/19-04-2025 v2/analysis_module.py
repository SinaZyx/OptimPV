...
class AnalysisModule:
    ...
    def calculate_financial_indicators(self, scenario_name, prix_revente=None):
        ...
        try:
            ...
            # --- AJOUT DEBUG PRODUCTION REF --- 
            print(f"DEBUG PROD: Somme Production Référence Horaire (ref_prod.sum()) = {ref_prod.sum():.2f}")
            if len(ref_prod) != 8760: print(f"AVERTISSEMENT PROD: ref_prod contient {len(ref_prod)} points, attendu 8760 pour horaire.")
            # --- FIN AJOUT DEBUG PRODUCTION REF ---
            ...
            for idx, year_num in enumerate(years):
                ...
                # --- AJOUT DEBUG PRODUCTION ANNUELLE (3 premières années) ---
                if idx < 3:
                    print(f"DEBUG PROD: Année {year_num}, Production Annuelle Calculée = {arrays['annual_production'][idx]:.2f}")
                # --- FIN AJOUT DEBUG PRODUCTION ANNUELLE ---
            ...
            # --- AJOUT CALCUL LCOE ---
            lcoe = None
            if wacc is not None and abs(wacc - (-1.0)) > 1e-9: # S'assurer que wacc est calculable et utilisable
                try:
                    # Actualiser la production annuelle
                    # --- AJOUT DEBUG SOMME PRODUCTION FINALE --- 
                    total_production_non_actualisee = arrays["annual_production"].sum()
                    print(f"DEBUG PROD: Somme Production Totale NON-Actualisée = {total_production_non_actualisee:.2f}")
                    # --- FIN AJOUT DEBUG SOMME PRODUCTION FINALE ---
                    discounted_production = [prod / ((1 + wacc) ** year) for year, prod in enumerate(arrays["annual_production"], 1) if (1 + wacc) ** year != 0]
                    total_discounted_production = sum(discounted_production)
                    
                    # Actualiser les OPEX annuels
                    discounted_opex = [op / ((1 + wacc) ** year) for year, op in enumerate(arrays["opex"], 1) if (1 + wacc) ** year != 0]
                    total_discounted_opex = sum(discounted_opex)
                    
                    # Coût total actualisé (CAPEX initial + OPEX actualisés)
                    # Utilisation du CAPEX brut initial pour un LCOE "asset-centric"
                    total_discounted_costs = capex + total_discounted_opex
                    
                    # --- AJOUT DEBUG PRINTS ---
                    print(f"DEBUG LCOE: CAPEX = {capex:.2f}")
                    print(f"DEBUG LCOE: Total Discounted OPEX = {total_discounted_opex:.2f}")
                    print(f"DEBUG LCOE: Total Discounted Production (kWh) = {total_discounted_production:.2f}")
                    print(f"DEBUG LCOE: WACC = {wacc}")
                    print(f"DEBUG LCOE: Total Discounted Costs = {total_discounted_costs:.2f}")
                    # --- FIN AJOUT DEBUG PRINTS ---
                    
                    if abs(total_discounted_production) > 1e-9:
                        lcoe = total_discounted_costs / total_discounted_production
                        if not np.isfinite(lcoe): lcoe = None # Vérifier si le résultat est fini
                    
                except (OverflowError, FloatingPointError, ValueError) as e_lcoe:
                    print(f"AVERTISSEMENT: Calcul LCOE a échoué - {e_lcoe}")
                    lcoe = None
            # --- FIN AJOUT CALCUL LCOE ---
...
# --- Méthodes d'Optimisation ---
    ...
    def display_financial_results(self, results, key_prefix=""):
        ...
        with col3:
            ...
            # --- AJOUT METRIQUE LCOE ---
            lcoe_value = f"{results.get('lcoe', 0.0):.4f} €/kWh" if results.get('lcoe') is not None else "N/A"
            st.metric("LCOE", lcoe_value, help="Levelized Cost of Energy : Coût actualisé moyen de production d'un kWh sur la durée de vie du projet (basé sur CAPEX brut + OPEX actualisés au WACC)." )
            # --- FIN AJOUT ---
        ...
        with st.expander("Détails des paramètres financiers"):
            ...
            with col3:
                ...
                # --- AJOUT METRIQUE INVESTISSEMENT NET ---
                st.metric("Investissement Net Fonds Propres", f"{results.get('net_equity_investment', 0.0):,.2f} €")
                # --- FIN AJOUT ---
        ...
        with tab1:
            ...
            with st.expander("Tableau détaillé des flux financiers"):
                # --- MODIFICATION POUR AFFICHER LE DF ENRICHI ---
                st.dataframe(df_financial, key=f"{key_prefix}financial_df_{scenario_name}_{unique_id}")
                # --- FIN MODIFICATION ---
                
            # --- AJOUT EXPANDER POUR FLUX TRI/VAN ---
            with st.expander("Flux de Trésorerie pour Calcul TRI/VAN"):
                cf_irr_npv = results.get('cash_flows_for_irr_npv', [])
                if cf_irr_npv:
                    # Créer un DataFrame pour un affichage propre
                    df_cf_irr = pd.DataFrame({
                        'Année': list(range(len(cf_irr_npv))),
                        'Flux Net (€)': cf_irr_npv
                    })
                    st.dataframe(df_cf_irr, key=f"{key_prefix}irr_npv_cf_df_{scenario_name}_{unique_id}")
                    st.caption("Année 0 = Investissement net initial ; Années suivantes = Free Cash Flow to Equity (FCFE)")
                    # --- AJOUT LÉGENDE TABLEAU DÉTAILLÉ --- 
                    st.caption("Formules clés du tableau détaillé : EBIT = EBITDA - Amortissement ; EBT = EBIT - Intérêts ; Impôts = max(0, EBT * Taux Imp.) ; Résultat Net = EBT - Impôts ; Free Cash Flow = Résultat Net + Amortissement - Principal Remboursé")
                    # --- FIN AJOUT LÉGENDE ---
                else:
                    st.warning("Flux de trésorerie pour TRI/VAN non disponibles.")
            # --- FIN AJOUT EXPANDER ---
        ...
        with tab2:
            ...
            with detail_tab1:
                ...
                # Créer un DataFrame pour les données de production (NÉCESSAIRE pour le calcul des taux ci-dessous)
                ...
                with st.expander("Taux d'autoconsommation et d'autoproduction"):
                    ...
                    # --- SUPPRESSION GRAPHIQUE DES TAUX --- 
                    # Graphique des taux
                    # fig_rates = go.Figure()
                    # 
                    ...
                    # --- AJOUT GRAPHIQUE CAMEMBERT RÉPARTITION PRODUCTION --- 
                    st.markdown("#### Répartition Moyenne de la Production Annuelle")
                    
                    # Récupérer le taux d'autoconsommation global
                    auto_conso_rate = results.get('autoconsumption_rate', 0.0)
                    surplus_rate = 1.0 - auto_conso_rate
                    
                    # Créer les données pour le camembert
                    labels = ['Autoconsommation', 'Surplus Vendu (OA)']
                    values = [auto_conso_rate * 100, surplus_rate * 100]
                    colors = ['#FFDA6A', '#1F77B4'] # Jaune pour Autoconso, Bleu pour Surplus
                    
                    fig_pie = go.Figure(data=[go.Pie(
                        labels=labels,
                        values=values,
                        marker_colors=colors,
                        hole=.3, # Pour un style 'donut'
                        textinfo='percent+label',
                        insidetextorientation='radial'
                    )])
                    
                    fig_pie.update_layout(
                        title_text="Répartition Moyenne Annuelle de l'Énergie Produite", # Utilisation de guillemets doubles
                        legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5)
                    )
                    
                    st.plotly_chart(fig_pie, use_container_width=True, key=f"{key_prefix}prod_pie_chart_{scenario_name}_{unique_id}")
                    # --- FIN AJOUT --- 
