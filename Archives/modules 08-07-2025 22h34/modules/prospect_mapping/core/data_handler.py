...
def search_and_geocode_address(address: str) -> dict:
    """
    Recherche et géocode une adresse pour obtenir ses coordonnées GPS.
    
    Args:
        address: Adresse complète à rechercher
        
    Returns:
        dict: Coordonnées et informations de l'adresse
    """
    try:
        url = CONFIG['URLS']['GEOCODING_API']
        ...
        logger.error(f"Erreur géocodage adresse '{address}': {e}")
        return {
            'success': False,
            'error': str(e)
        }
...
def get_all_dpe_around_point(latitude: float, longitude: float, radius_m: int = 500) -> pd.DataFrame:
    """
    Récupère tous les DPE dans un rayon autour d'un point GPS.
    ...
    Returns:
        pd.DataFrame: Tous les DPE trouvés avec leurs informations
    """
    try:
        url = CONFIG['APIS']['DPE_API_URL']
        ...
        lon_delta = radius_m / (111000 * np.cos(np.radians(latitude)))
        
        # D'abord récupérer le code postal via reverse geocoding
        try:
            geocode_url = "https://api-adresse.data.gouv.fr/reverse/"
            geo_params = {'lon': longitude, 'lat': latitude}
            ...
            if geo_response.status_code == 200:
                ...
                if features:
                    ...
                    postcode = properties.get('postcode', '')
                    city = properties.get('city', '')
                    logger.info(f"Localisation identifiée: {city} ({postcode})")
        except Exception as e:
            logger.warning(f"Erreur reverse geocoding: {e}")
            postcode = None
        
        # Utiliser le code postal pour une première recherche
        if postcode:
            # Recherche par code postal - utiliser 'q' avec juste le code postal
            params = {
                'q': postcode,
                'select': 'Coordonnée_cartographique_X_(BAN),Coordonnée_cartographique_Y_(BAN),'
                         'Etiquette_DPE,Etiquette_GES,Date_établissement_DPE,'
                         'Surface_habitable_logement,'
                         'Type_bâtiment,Année_construction,'
                         'Adresse_(BAN),Code_postal_(BAN)',
                'size': 5000  # Limiter pour éviter timeout
            }
        else:
            # Fallback: recherche sans filtre géographique (on filtrera après)
            params = {
                'select': 'Coordonnée_cartographique_X_(BAN),Coordonnée_cartographique_Y_(BAN),'
                         'Etiquette_DPE,Etiquette_GES,Date_établissement_DPE,'
                         'Consommation_énergétique,Surface_habitable_logement,'
                         'Type_bâtiment,Année_construction,'
                         'Adresse_(BAN),Code_postal_(BAN),Libellé_commune_(BAN)',
                'size': 1000
            }
        
        logger.info(f"Requête API DPE avec params: {params}")
        response = requests.get(url, params=params, timeout=10)
        response.raise_for_status()
        
        data = response.json()
        results = data.get('results', [])
        logger.info(f"API DPE retourne {len(results)} résultats")
        
        if results:
            df_dpe = pd.DataFrame(results)
            
            # Convertir les coordonnées Lambert 93 (X,Y) en latitude/longitude
            # Les coordonnées BAN sont en Lambert 93 (EPSG:2154)
            if 'Coordonnée_cartographique_X_(BAN)' in df_dpe.columns and 'Coordonnée_cartographique_Y_(BAN)' in df_dpe.columns:
                try:
                    from pyproj import Transformer
                    transformer = Transformer.from_crs("EPSG:2154", "EPSG:4326", always_xy=True)
                    
                    # Convertir X,Y en lon,lat
                    coords = df_dpe.apply(
                        lambda row: transformer.transform(
                            row['Coordonnée_cartographique_X_(BAN)'], 
                            row['Coordonnée_cartographique_Y_(BAN)']
                        ) if pd.notna(row['Coordonnée_cartographique_X_(BAN)']) and pd.notna(row['Coordonnée_cartographique_Y_(BAN)']) else (None, None),
                        axis=1
                    )
                    
                    df_dpe['longitude'] = coords.apply(lambda x: x[0] if x[0] else None)
                    df_dpe['latitude'] = coords.apply(lambda x: x[1] if x[1] else None)
                    
                except ImportError:
                    logger.warning("pyproj non disponible, retour des DPE sans filtrage par distance")
                    # Sans pyproj, on ne peut pas convertir précisément les coordonnées
                    # On retourne tous les DPE du code postal sans filtrage par distance
                    df_dpe['longitude'] = None
                    df_dpe['latitude'] = None
                    df_dpe['distance_m'] = 999999  # Distance fictive pour éviter les erreurs
            
            # Renommer les colonnes pour cohérence avec notre système
            column_mapping = {
                'Etiquette_DPE': 'dpe_classe_energie',
                'Etiquette_GES': 'dpe_classe_ges',
                'Consommation_énergétique': 'dpe_consommation',
                'Adresse_(BAN)': 'adresse',
                'Libellé_commune_(BAN)': 'commune',
                'Type_bâtiment': 'type_batiment',
                'Surface_habitable_logement': 'surface_habitable',
                'Année_construction': 'annee_construction'
            }
            
            df_dpe = df_dpe.rename(columns=column_mapping)
            
            # Calculer la distance si on a des coordonnées valides
            if df_dpe['latitude'].notna().any() and df_dpe['longitude'].notna().any():
                from geopy.distance import distance
                df_dpe['distance_m'] = df_dpe.apply(
                    lambda row: distance(
                        (latitude, longitude),
                        (row['latitude'], row['longitude'])
                    ).meters if pd.notna(row['latitude']) and pd.notna(row['longitude']) else float('inf'),
                    axis=1
                )
                
                # Filtrer par rayon
                df_dpe = df_dpe[df_dpe['distance_m'] <= radius_m]
                
                # Trier par distance
                df_dpe = df_dpe.sort_values('distance_m')
            else:
                # Sans coordonnées, on ne peut pas filtrer par distance
                # On retourne les premiers résultats du code postal
                logger.warning(f"Impossible de filtrer par distance sans conversion de coordonnées")
                if 'distance_m' not in df_dpe.columns:
                    df_dpe['distance_m'] = 999999  # Distance fictive
            
            logger.info(f"Trouvé {len(df_dpe)} DPE dans un rayon de {radius_m}m")
            return df_dpe
            
        else:
            logger.info(f"Aucun DPE trouvé dans un rayon de {radius_m}m")
            return pd.DataFrame()
            
    except Exception as e:
        logger.error(f"Erreur récupération DPE autour du point: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        return pd.DataFrame()

def create_dpe_color_map(classe_energie: str) -> list:
    """
    Retourne la couleur RGB pour une classe énergétique DPE.
    
    Args:
        classe_energie: Classe énergétique (A, B, C, D, E, F, G)
        
    Returns:
        list: Couleur [R, G, B, A] pour PyDeck
    """
    color_map = {
