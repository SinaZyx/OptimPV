...
def display_cash_flow_statement_restructured(results: dict | None, selected_year: int):
    ...
    placement_tresorerie_active = config_globale.get('placement_tresorerie_active', False)
    ...
    # --- PASSE 1.5: Calcul des Lignes "calculated" avec lambda ---
    for item_cfg in config_structure:
        item_type = item_cfg.get('type', 'data')
        if item_type == 'calculated':
            item_id = item_cfg['id']
            calculation_func = item_cfg.get('calculation')
            if calculation_func and isinstance(calculation_func, str):
                try:
                    # Évaluer la fonction lambda
                    calc_lambda = eval(calculation_func)
                    
                    # Créer l'entrée dans processed_data_for_table si elle n'existe pas
                    if item_id not in processed_data_for_table:
                        processed_data_for_table[item_id] = {
                            'label': item_cfg.get('display_name', 'Calculated'),
                            'raw_monthly_for_calc': [0.0] * num_months_to_display,
                            'monthly_for_display': [0.0] * num_months_to_display,
                            'style_class': item_cfg.get('style_class', ''),
                            'decimals': item_cfg.get('decimals', 0),
                            'format': item_cfg.get('format', ''),
                            'tooltip': item_cfg.get('tooltip', '')
                        }
                    
                    # Appliquer le calcul pour chaque mois
                    for month_idx, month_name in enumerate(months_display_order):
                        month_row = df_year_filtered[df_year_filtered.index.strftime('%b').str.capitalize() == month_name].iloc[0] if not df_year_filtered[df_year_filtered.index.strftime('%b').str.capitalize() == month_name].empty else pd.Series()
                        if not month_row.empty:
                            row_dict = month_row.to_dict()
                            calculated_value = calc_lambda(row_dict)
                            processed_data_for_table[item_id]['raw_monthly_for_calc'][month_idx] = calculated_value
                            processed_data_for_table[item_id]['monthly_for_display'][month_idx] = calculated_value
                except Exception as e:
                    logger.error(f"Erreur lors du calcul lambda pour {item_id}: {e}")
    ...
    for item_cfg_annual in config_structure:
        ...
        # Définir les mots-clés pour identifier les soldes
        solde_keywords_mcf = [
            # Mots-clés génériques
            'placement', 'encours', 'tresorerie', 'solde', 'cumul', 'balance', 'fin_mois',
            
            # Colonnes exactes des placements
            ...
            'cash_opening', 'cash_closing', 'cash_opening_balance', 'cash_closing_balance',
            
            # NOUVEAU: IDs de la section VI
            'tresorerie_totale_fin', 'reserve_minimum', 'tresorerie_disponible_calc',
            'fonds_reserve_onduleur_detail', 'placements_tva_detail', 'placements_excedents_detail',
            'total_placements_calc', 'ratio_securite',
            
            # Colonnes exactes utilisées pour la section VI
            'reserve_minimum_requise', 'fonds_reserve_onduleur_total',
            'total_placements_tva', 'solde_placement_excedents_cumul',
            
            # Variantes avec espaces
            'total placements', 'trésorerie non placée', 'solde placement'
        ]

        # Fonction helper locale pour déterminer si c'est un solde
        def is_solde_item(item_id, display_name=''):
            """Détermine si un item est un solde basé sur son ID ou display_name"""
            check_text = f"{item_id} {display_name}".lower()
            return any(keyword in check_text for keyword in solde_keywords_mcf)
        ...
        if item_type_annual == 'calculated_balance_item' and item_cfg_annual.get('calculation_logic') == 'opening_balance':
            ...
            # CORRECTION : Distinguer soldes vs flux selon les mots-clés
            if is_solde_item(item_id_annual, item_cfg_annual.get('display_name', '')):
                # Pour les soldes : dernière valeur non-nulle
                non_null_values = [v for v in monthly_values_for_annual_sum if v is not None and v != 0]
                processed_data_for_table[item_id_annual]['annual_total'] = non_null_values[-1] if non_null_values else 0.0
            else:
                # Pour les flux : somme (comportement actuel)
    ...
    for item_cfg_display in config_structure:
        ...
        # NOUVEAU: Filtrage conditionnel des éléments de placement
        item_id_check = item_cfg_display.get('id', '')
        ...
        # Si les placements sont désactivés et que c'est un élément lié aux placements, ignorer
        if not placement_tresorerie_active and item_id_check in placement_related_ids:
            continue
        
        # Masquer aussi les sections/headers de placement si désactivé
        if not placement_tresorerie_active and item_type_display == 'section_header':
            display_lower = display_name_html.lower()
            if any(keyword in display_lower for keyword in ['placement', 'trésorerie', 'intérêt']):
                continue
        ...
        # === LOGGING TVA PLACEMENT POUR DEBUG ===
        if item_id_display in ['placement_tva_mois', 'placement_provision_onduleur_mois', 'interets_capitalises_mois', 
                              'interets_courus_cumul', 'tresorerie_non_placee_fin', 'total_placements_fin', 'vat_net_payment']:
            # Importer la fonction de log si nécessaire
            try:
                from modules.engine_module.treasury_placement import log_tva_placement
                
                # Trouver les valeurs non nulles
                non_zero_months = [(i, val) for i, val in enumerate(monthly_values_to_render) if val != 0]
                if non_zero_months:
                    log_data = {
                        "Ligne": display_name_html.strip(),
                        "Année": selected_year,
                        "Valeurs_non_nulles": {f"Mois_{i+1}": f"{val:,.2f}€" for i, val in non_zero_months},
                        "Total_annuel": f"{annual_total_to_render:,.2f}€"
                    }
                    log_tva_placement(f"AFFICHAGE TABLEAU - {item_id_display}", log_data)
            except Exception as e:
                logger.warning(f"Impossible de logger TVA placement: {e}")
        ...
        for val_month_render in monthly_values_to_render:
            ...
            # Récupérer le format spécial si défini
            format_type = processed_data_for_table[item_id_display].get('format', '')
            formatted_val_html = format_value(val_to_format_render, '', 0, "-", format_type)
        ...
        formatted_annual_html = format_value(annual_val_to_format_render, '', 0, "-", format_type)
    ...
    # --- ALERTES DE SÉCURITÉ TRÉSORERIE ---
    # Analyser la trésorerie disponible et afficher des alertes
    months_sous_minimum = []
    notes_remplacement = []
    
    for month_idx, month_name in enumerate(months_display_order):
        month_row = df_year_filtered[df_year_filtered.index.strftime('%b').str.capitalize() == month_name]
        if not month_row.empty:
            month_data = month_row.iloc[0]
            
            # Vérifier la trésorerie disponible
            tresorerie_fin = month_data.get('Solde_Tresorerie_Fin_Mois', 0)
            reserve_min = month_data.get('Reserve_Minimum_Requise', 0)
            if reserve_min > 0:
                tresorerie_disponible = tresorerie_fin - reserve_min
                if tresorerie_disponible < 0:
                    months_sous_minimum.append(month_name)
            
            # Détecter le remplacement onduleur
            if abs(month_data.get('Remplacement_Onduleur_Paye', 0)) > 0:
                liberation = month_data.get('Fonds_Reserve_Onduleur_Liberation', 0)
                cout_onduleur = abs(month_data.get('Remplacement_Onduleur_Paye', 0))
                if abs(liberation) > 0:
                    notes_remplacement.append(f"ℹ️ Remplacement onduleur en {month_name} {selected_year}: Coût {cout_onduleur:,.0f}€, intégralement financé par le fonds de réserve ({liberation:,.0f}€)")
    
    # Afficher les alertes
    if months_sous_minimum:
        st.warning(f"⚠️ Attention : Trésorerie sous le minimum de sécurité en {', '.join(months_sous_minimum)} {selected_year}")
    
    # Afficher les notes
    for note in notes_remplacement:
        st.info(note)
    
    # Indicateur global de santé financière
    if 'Reserve_Minimum_Requise' in df_year_filtered.columns:
        tresorerie_moy = df_year_filtered['Solde_Tresorerie_Fin_Mois'].mean()
        reserve_moy = df_year_filtered['Reserve_Minimum_Requise'].mean()
        if reserve_moy > 0:
            ratio_moyen = tresorerie_moy / reserve_moy
            if ratio_moyen >= 1.5:
                st.success(f"✅ Santé financière : Excellente (ratio moyen {ratio_moyen:.1f})")
            elif ratio_moyen >= 1.0:
                st.info(f"ℹ️ Santé financière : Correcte (ratio moyen {ratio_moyen:.1f})")
            else:
                st.error(f"❌ Santé financière : Préoccupante (ratio moyen {ratio_moyen:.1f})")
