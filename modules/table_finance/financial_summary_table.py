...
def display_project_summary(results: dict, table_map: dict):
    ...
    capex_glob = results.get('capex_scenario_simule_initial')
    subvention_glob = results.get('total_subvention')
    net_equity_glob = results.get('net_equity_investment')
...
def display_financial_summary_table(results: dict | None):
    ...
    tab_synthese, tab_tresorerie = st.tabs(["📊 Compte de Résultat", "🏦 Flux de Trésorerie Mensuels"])
                        ...
                        try:
                            ...
                            try:
                                ...
                                def get_annual_total(df, display_key, default=np.nan):
                                    ...
                                    # CORRECTION : Distinguer les soldes (dernière valeur) des flux (somme)
                                    if display_key in df.index:
                                        # Identifier les lignes de type "solde" ou "stock"
                                        solde_keywords = [
                                            'solde', 'total placements', 'trésorerie non placée', 'encours',
                                            'total des placements', 'tresorerie non placee'
                                        ]
                                        
                                        # Vérifier si c'est un solde (en minuscules pour la comparaison)
                                        is_solde = any(keyword in display_key.lower() for keyword in solde_keywords)
                                        
                                        if is_solde:
                                            # Pour les soldes : prendre la dernière valeur non-NaN (solde de fin de période)
                                            monthly_values = df.loc[display_key, numeric_month_cols_year].dropna()
                                            return monthly_values.iloc[-1] if not monthly_values.empty else default
                                        else:
                                            # Pour les flux : conserver la logique de somme
