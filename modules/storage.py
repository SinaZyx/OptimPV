...
class StorageModule:
    ...
    def _show_save_dialog(self):
        ...
        with st.form("save_project_form"):
            ...
            # Tags (TODO: implémenter système de tags)
            # tags = st.multiselect("Tags", options=self._get_available_tags())
            
            col1, col2 = st.columns(2)
            
            with col1:
                save_button = st.form_submit_button("💾 Sauvegarder", type="primary", use_container_width=True)
            
            with col2:
                cancel_button = st.form_submit_button("Annuler", use_container_width=True)
            
            if save_button and name:
                with st.spinner("Sauvegarde en cours..."):
                    # Utiliser la méthode save_current_project intégrée
                    project_id = self.save_current_project(name, description)
...
"""
    ...
    def _show_economic_comparison(self, comparison_data):
        ...
        if not economic1 or not economic2:
            st.info("Données économiques non disponibles pour la comparaison")
            return
    ...
    def _show_optimization_comparison(self, comparison_data):
        ...
        if not opt1 or not opt2:
            st.info("Résultats d'optimisation non disponibles pour la comparaison")
            return
    ...
    def _show_monte_carlo_comparison(self, comparison_data):
        ...
        if not mc1 or not mc2:
            st.info("Simulations Monte Carlo non disponibles pour la comparaison")
            return
