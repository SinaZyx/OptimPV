...
class EquityCalculations:
    ...
    def calculate_fcfe_professional(self, monthly_results_df, subvention_totale_projet):
        ...
        for idx, row in monthly_results_df.iterrows():
            ...
            # AJOUT : Prise en compte du remplacement onduleur
            # Le paiement de l'onduleur est un flux d'investissement qui affecte les actionnaires
            remplacement_onduleur = row.get('Remplacement_Onduleur_Paye', 0)
            if remplacement_onduleur != 0:
                fcfe_operational += remplacement_onduleur  # Négatif si paiement
                
            # La libération du fonds est un flux positif
            liberation_fonds = row.get('Fonds_Reserve_Onduleur_Liberation', 0)
            if liberation_fonds != 0:
                fcfe_operational += liberation_fonds  # Positif si libération
            
            # AJOUT CRITIQUE : Intégrer TOUS les intérêts de placement dans les flux
            # Ces intérêts sont des produits financiers qui appartiennent aux actionnaires
            interets_totaux = row.get('Interets_Totaux_Mensuels', 0)
            
            # Les intérêts augmentent le cash disponible pour les actionnaires
            if interets_totaux != 0:
                fcfe_operational += interets_totaux
                
            # Pour compatibilité, garder aussi les colonnes individuelles
            interets_placements_mensuels = row.get('Interets_Placements_Mensuels', 0)
            interets_fonds_reserve = row.get('Fonds_Reserve_Onduleur_Interets', 0)
            ...
            fcfe_details.append({
                ...
                'components': {
                    ...
                    'interets_placements': interets_placements_mensuels,
                    'interets_fonds_reserve': interets_fonds_reserve,
