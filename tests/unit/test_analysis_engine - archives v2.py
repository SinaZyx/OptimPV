...
def test_zero_production_year_impact_multi_year(
    base_config_real, scenarios_real, sites_config_real, data_for_temporal_test_fixture # Utilise la nouvelle fixture multi-années
):
    ...
    # 2. Cas avec une année de production nulle
    site_data_modified = copy.deepcopy(data_for_temporal_test_fixture)
    df_site_to_modify = site_data_modified[site_key_temporal]

    # S'assurer que la colonne Temps est de type datetime pour la modification
    if not pd.api.types.is_datetime64_any_dtype(df_site_to_modify['Temps']):
        df_site_to_modify['Temps'] = pd.to_datetime(df_site_to_modify['Temps'], errors='coerce')
        df_site_to_modify.dropna(subset=['Temps'], inplace=True)
    
    # Déterminer une année d'exploitation pour mettre la production à zéro
    # Par exemple, la deuxième année pleine d'exploitation
    date_debut_exploitation_dt = pd.to_datetime(cfg["date_debut_ppa"])
    year_to_modify = date_debut_exploitation_dt.year + 1 # Deuxième année d'exploitation

    # Filtrer sur l'année et mettre la production à zéro
    # Il faut s'assurer que df_site_to_modify a bien des données pour cette année
    original_sum_prod_year_to_modify = df_site_to_modify[pd.to_datetime(df_site_to_modify['Temps']).dt.year == year_to_modify]['production_kwh'].sum()
    
    if original_sum_prod_year_to_modify > 0 : # S'il y avait de la production à modifier
        df_site_to_modify.loc[pd.to_datetime(df_site_to_modify['Temps']).dt.year == year_to_modify, 'production_kwh'] = 0.0
        modified_sum_prod_year_to_modify = df_site_to_modify[pd.to_datetime(df_site_to_modify['Temps']).dt.year == year_to_modify]['production_kwh'].sum()
        print(f"INFO: Production pour l'année {year_to_modify} mise à zéro. Somme prod avant: {original_sum_prod_year_to_modify:.0f}, après: {modified_sum_prod_year_to_modify:.0f}")
        assert math.isclose(modified_sum_prod_year_to_modify, 0.0), "La production de l'année modifiée devrait être nulle."
    else:
        pytest.skip(f"Pas de production à modifier pour l'année {year_to_modify} dans la fixture de données.")


    engine_zero_prod = AnalysisEngine(cfg, scenarios_real, site_data_modified)
    ...
    assert lcoe_zero_prod > lcoe_base, "LCOE avec une année de production nulle devrait être plus élevé."
...
def test_golden_sample_comparison(
    ...
    print(f"\n--- Test Golden Sample ---")
    print(f"Configuration Utilisée pour Golden Sample:")
    print(f"  CAPEX: {cfg['capex_scenario']}")
    print(f"  Prix Vente: {cfg['prix_vente_initial_slider_fallback']}")
    print(f"  Production Annuelle: 55000 kWh")
    print(f"  Consommation Annuelle: 30000 kWh")
    print(f"  OPEX Annuels: 1000 €")
    print(f"  Amortissement: Linéaire sur {cfg['amortissement_duree']} ans")
    print(f"  Coût des Fonds Propres: {cfg['cout_fonds_propres']}%")

    # --- EXPORT CSV DES FLUX DU MOTEUR ---
    export_filename = "golden_sample_engine_cashflows.csv"
    df_monthly_engine.to_csv(export_filename)
    print(f"Flux mensuels du moteur exportés vers : {os.path.abspath(export_filename)}")
    print("Ouvrez ce CSV et comparez-le à votre modèle Excel 'Golden Sample'.")

    # --- VALEURS CALCULÉES PAR EXCEL (À RENSEIGNER MANUELLEMENT APRÈS CALCUL DANS EXCEL) ---
    # Remplacez ces valeurs par celles de VOTRE Golden Sample Excel
    excel_van_projet_attendu = 6765.0   # Exemple, à remplacer
    excel_tri_projet_attendu = 0.1056   # Exemple (10.56%), à remplacer
    excel_lcoe_attendu = 0.1362         # Exemple, à remplacer
    excel_payback_projet_attendu = 2.8  # Exemple, à remplacer
    
    # Indicateurs annuels (Exemple pour la 1ère année complète - Année 1 de l'exploitation)
    # L'année 0 dans les flux annuels Excel est souvent juste le CAPEX.
    # L'année 1 est la première année d'exploitation.
    excel_an1_revenus_total = 8250.0  # (55000 kWh * 0.15 €/kWh)
    excel_an1_ebitda = 6950.0     # 8250 - 1000 (OPEX) - 300 (TURPE)
    excel_an1_ebt = -3050.0       # 6950 - 10000 (Amort)
    excel_an1_is = 0.0            # EBT < 0 ou tranche PME à 15%
    excel_an1_resultat_net = -3050.0
    excel_an1_fcfe = 6950.0       # -3050 (RN) + 10000 (Amort)
    excel_an1_ocf_projet = 6950.0 # Pour ce cas simple sans IS et NOPAT = EBITDA
                                 # NOPAT = EBIT*(1-T) = -3050 * (1-0) = -3050. OCF = -3050 + 10000 = 6950
    ...
    print(f"  VAN Projet: Moteur={engine_van_projet:,.0f} €, Excel Attendu={excel_van_projet_attendu:,.0f} €")
    print(f"  TRI Projet: Moteur={engine_tri_projet*100:.2f}%, Excel Attendu={excel_tri_projet_attendu*100:.2f}%")
    print(f"  LCOE: Moteur={engine_lcoe:.4f}, Excel Attendu={excel_lcoe_attendu:.4f}")
    print(f"  Payback Projet: Moteur={engine_payback_projet:.1f} ans, Excel Attendu={excel_payback_projet_attendu:.1f} ans")

    # Assertions (avec une tolérance pour les flottants)
    assert math.isclose(engine_van_projet, excel_van_projet_attendu, rel_tol=0.001, abs_tol=1.0) # Tolérance relative 0.1%, absolue 1€
    assert math.isclose(engine_tri_projet, excel_tri_projet_attendu, rel_tol=0.001, abs_tol=0.0001) # Tolérance relative 0.1%, absolue 0.01%
    assert math.isclose(engine_lcoe, excel_lcoe_attendu, rel_tol=0.001, abs_tol=0.0001)
    ...
    # --- Comparaison des Agrégats Annuels (Exemple pour Année 1 d'exploitation) ---
    # L'index de df_monthly_engine est Datetime. L'année 1 d'exploitation correspond à cfg["date_debut_ppa"]
    ...
    if not df_an1_engine.empty:
        ...
        engine_an1_ebt = df_an1_engine['EBT'].sum() # EBT du moteur est après report déficitaire mensuel
        engine_an1_is_paye = df_an1_engine['Total_IS_Decaisse_Mois'].sum() # IS effectivement payé
        engine_an1_resultat_net = df_an1_engine['Resultat_Net'].sum() # RN moteur = EBT (après report) - IS payé
        ...
        print(f"  Revenus: Moteur={engine_an1_revenus:,.0f}, Excel={excel_an1_revenus_total:,.0f}")
        print(f"  EBITDA:  Moteur={engine_an1_ebitda:,.0f}, Excel={excel_an1_ebitda:,.0f}")
        # Pour EBT, IS, RN, FCFE, OCF, la comparaison directe est plus complexe à cause du report de déficit mensuel
        # et du calcul de l'IS mensuel dans le moteur Python. Excel a souvent un calcul d'IS annuel.
        # Il faut s'assurer que le calcul Excel reproduit la logique fiscale mensuelle (ou comparer les totaux avant IS).
        print(f"  EBT (Moteur, après report mensuel): {engine_an1_ebt:,.0f} vs Excel (avant IS): {excel_an1_ebt:,.0f}")
        print(f"  IS Payé (Moteur): {engine_an1_is_paye:,.0f} vs Excel (calculé): {excel_an1_is:,.0f}")
        print(f"  Résultat Net (Moteur): {engine_an1_resultat_net:,.0f} vs Excel: {excel_an1_resultat_net:,.0f}")
        print(f"  FCFE (Moteur): {engine_an1_fcfe:,.0f} vs Excel: {excel_an1_fcfe:,.0f}")
        print(f"  OCF Projet (Moteur): {engine_an1_ocf_projet:,.0f} vs Excel: {excel_an1_ocf_projet:,.0f}")


        assert math.isclose(engine_an1_revenus, excel_an1_revenus_total, rel_tol=0.001)
        assert math.isclose(engine_an1_ebitda, excel_an1_ebitda, rel_tol=0.001)
        # Pour les lignes suivantes, la tolérance doit être plus grande ou la logique Excel doit être alignée.
        # assert math.isclose(engine_an1_ebt, excel_an1_ebt, rel_tol=0.01, abs_tol=50) # EBT peut différer à cause des reports
        # assert math.isclose(engine_an1_is_paye, excel_an1_is, rel_tol=0.01, abs_tol=50) 
        # assert math.isclose(engine_an1_resultat_net, excel_an1_resultat_net, rel_tol=0.01, abs_tol=50)
        assert math.isclose(engine_an1_fcfe, excel_an1_fcfe, rel_tol=0.01, abs_tol=50)
        assert math.isclose(engine_an1_ocf_projet, excel_an1_ocf_projet, rel_tol=0.01, abs_tol=50)
    else:
        print(f"AVERTISSEMENT: Aucune donnée pour l'année {annee_1_exploitation} dans les résultats du moteur.")
        
    print("--- Test Golden Sample Terminé ---")